...
class TestSupervisorStateInitialization:
    ...
    @pytest.mark.asyncio
    async def test_entry_router_initializes_supervisor_state_on_resumption(
        self, workflow_graph, complete_state
    ):
    ...
    async def test_entry_router_handles_empty_sections(self, workflow_graph):
        ...
        state = AgentState(
            ...
            job_description_data=JobDescriptionData(
                raw_text="Sample job description", parsed_requirements=["Python"]
            ),
            # Workflow Control & Granular Processing
            current_section_key=None,
            current_section_index=None,
    ...
    @pytest.mark.asyncio
    async def test_cv_parser_node_initializes_supervisor_state_after_parsing(
        self, workflow_graph, mock_agents
    ):
        ...
        sample_cv = StructuredCV(sections=[Section(name="Test Section", items=[item])])
