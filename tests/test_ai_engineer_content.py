...
• Flat hierarchies and opportunities for further development"""
    ...
    def _create_test_structured_cv(self):
        ...
        summary_section.items.append(Item(
        ...
        for skill in ["Python", "SQL", "Data Analysis", "Machine Learning", "Data Visualization", "Problem Solving"]:
        ...
        data_analyst_subsection = Subsection(
            name="Trainee Data Analyst"
        )
        data_analyst_subsection.items.append(Item(
        ...
        it_trainer_subsection = Subsection(
            name="IT trainer"
        )
        it_trainer_subsection.items.append(Item(
        ...
        erp_project_subsection.items.append(Item(
    ...
    def test_key_qualifications_content(self):
        ...
        self.agent._generate_key_qualifications(
            ...
            job_focus=self.agent._extract_job_focus(self.ai_engineer_jd, self.research_results)
        )
        ...
        key_qual_prompts = [p for p in self.captured_prompts if "key qualifications" in p.lower()]
        ...
        # Check that the prompt included job focus areas
        self.assertIn("JOB FOCUS AREAS", sample_prompt)
        self.assertIn("Primary skills needed", sample_prompt)
        ...
        ai_related_terms = ["AI", "Machine Learning", "Deep Learning", "PyTorch", "TensorFlow", "ML", "LLM"]
        has_ai_qualification = any(any(term.lower() in qual.lower() for term in ai_related_terms) for qual in qualifications)
        self.assertTrue(has_ai_qualification, "No AI-related qualification found in generated content")
    ...
    def test_executive_summary_content(self):
        ...
        # Examine the captured prompts to verify job description was included
        summary_prompts = [p for p in self.captured_prompts if "executive summary" in p.lower() or "profile" in p.lower()]
        ...
        # Check that the prompt included job focus areas
        self.assertIn("JOB REQUIREMENTS FOCUS", sample_prompt)
        ...
        self.assertTrue(has_ai_focus, "No AI focus found in generated executive summary")
    ...
    def test_experience_bullet_point_content(self):
        ...
        # Examine the captured prompts to verify job description was included
        experience_prompts = [p for p in self.captured_prompts if "professional experience" in p.lower()]
        self.assertGreater(len(experience_prompts), 0)
        
        # Check prompt contains AI Engineer information
        sample_prompt = experience_prompts[0]
        self.assertIn("AI", sample_prompt)
        ...
        ai_related_terms = ["AI", "Machine Learning", "Deep Learning", "PyTorch", "TensorFlow", "ML", "LLM"]
        ai_focused_bullets = [bullet for bullet in all_bullets if any(term.lower() in bullet.lower() for term in ai_related_terms)]
        
        logger.info("AI-focused bullet points: %s", ai_focused_bullets)
        self.assertGreater(len(ai_focused_bullets), 0, "No AI focus found in generated experience bullet points")
    ...
    def test_full_cv_generation(self):
        ...
        ai_terms = ["AI", "Machine Learning", "Deep Learning", "TensorFlow", "PyTorch", "NLP", "LLM"]
        ...
        self.assertGreater(ai_term_count, 3, f"AI terms only appeared {ai_term_count} times across the CV")
