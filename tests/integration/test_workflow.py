...
def main():
    ...
    dynamic_sections = [section for section in structured_cv.sections if section.content_type == "DYNAMIC"]
    static_sections = [section for section in structured_cv.sections if section.content_type == "STATIC"]
    ...
    try:
        ...
        if "structured_cv" in result and result["structured_cv"]:
            ...
            logger.info(f"Final structured CV has {len(structured_cv.sections)} sections")
            ...
            for section in structured_cv.sections:
                if section.content_type == "DYNAMIC":
                    ...
                    if section.name == "Executive Summary" and section.items:
                        has_content = any(item.content.strip() for item in section.items)
