...
from src.models.data_models import StructuredCV, JobDescriptionData, Section, Item, ItemStatus, ItemType, PersonalInfo, WorkExperience, Education, Skill
...
def create_sample_structured_cv() -> StructuredCV:
    ...
    project_subsection = Subsection(
        name="E-commerce Platform | Personal Project"
    )
...
async def test_pdf_generation():
    ...
    try:
        ...
        logger.info(f"CV sections: {[section.name for section in structured_cv.sections]}")
        ...
        if "pdf_path" in result:
            ...
            return {"success": False, "error": "No PDF path in result", "result": result}
...
async def test_pdf_with_custom_data(cv_data: Dict[str, Any] = None, job_data: Dict[str, Any] = None):
