...
class TestPDFWorkflowIntegration:
        ...
        cv_data = StructuredCV(
            ...
            sections=[
                CVSection(
                    ...
                    items=[
                        CVItem(content="Data Science"),
                        CVItem(content="Python Programming"),
                        CVItem(content="Statistical Analysis"),
                        CVItem(content="Machine Learning"),
                        CVItem(content="SQL Databases")
                    ]
                ),
                CVSection(
                    name="Professional Experience",
                    items=[
                        CVItem(content="Developed predictive models for customer behavior analysis"),
                        CVItem(content="Implemented automated data pipelines processing 1M+ records daily"),
                        CVItem(content="Led cross-functional team of 8 members in data migration project")
                    ]
                ),
                CVSection(
                    name="Project Experience",
                    items=[
                        CVItem(content="Built recommendation system increasing user engagement by 25%"),
                        CVItem(content="Created real-time dashboard for executive reporting")
                    ]
                )
            ]
        )
    ...
    def test_formatter_node_integration(self, mock_css, mock_html, mock_env, mock_get_config, sample_cv_state):
        ...
        mock_template = Mock()
        mock_template.render.return_value = "<html><body>Integration Test CV</body></html>"
        mock_jinja_env = Mock()
    ...
    @patch('src.agents.formatter_agent.HTML')
    @patch('src.agents.formatter_agent.CSS')
    def test_end_to_end_pdf_generation_flow(self, mock_css, mock_html, mock_env, mock_get_config, sample_cv_state):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert result_state.structured_cv.metadata["name"] == "Jane Smith"
            assert result_state.structured_cv.metadata["email"] == "jane.smith@example.com"
            assert len(result_state.structured_cv.sections) == 3
            
            # Verify Key Qualifications section
            key_quals = next(s for s in result_state.structured_cv.sections if s.name == "Key Qualifications")
            ...
            prof_exp = next(s for s in result_state.structured_cv.sections if s.name == "Professional Experience")
            ...
            mock_html.assert_called_once_with(string=expected_html, base_url=str(Path(temp_dir) / "src" / "templates"))
