...
class TestItemProcessorLLMServiceIntegration:
    ...
    def mock_llm_service(self):
        ...
        with patch('src.services.llm_service.genai') as mock_genai:
            ...
            service = EnhancedLLMService(api_key="test-api-key")
    ...
    @pytest.mark.asyncio
    async def test_successful_qualification_processing(self, item_processor, sample_qualification_item, job_context, mock_llm_service):
        """Test successful qualification processing with real service integration."""
        # Mock successful API response
        mock_response = Mock()
        mock_response.text = "Enhanced Python qualification tailored for Senior Python Developer role"
        mock_llm_service.llm.generate_content_async = Mock(return_value=mock_response)
        
        result = await item_processor.process_item(sample_qualification_item, job_context)
        
        assert result is True
        assert sample_qualification_item.metadata.status == ProcessingStatus.COMPLETED
    ...
    @pytest.mark.asyncio
    async def test_successful_experience_processing(self, item_processor, sample_experience_item, job_context, mock_llm_service):
        ...
        assert result is True
        assert sample_experience_item.metadata.status == ProcessingStatus.COMPLETED
        
        # Verify LLM service was called
        mock_llm_service.llm.generate_content_async.assert_called_once()

    @pytest.mark.asyncio
    async def test_llm_service_failure_handling(self, item_processor, sample_qualification_item, job_context, mock_llm_service):
        ...
        with patch.object(mock_llm_service, 'generate_content') as mock_generate:
            ...
            assert sample_qualification_item.metadata.status == ProcessingStatus.FAILED
            
            # Verify ItemProcessor called LLM service exactly once (no additional retries)
            mock_generate.assert_called_once()

    @pytest.mark.asyncio
    async def test_configuration_error_propagation(self, item_processor, sample_qualification_item, job_context):
    ...
    @pytest.mark.asyncio
    async def test_multiple_items_processing(self, item_processor, job_context, mock_llm_service):
        ...
        assert all(results)
        assert all(item.metadata.status == ProcessingStatus.COMPLETED for item in items)
        
        # Verify LLM service was called for each item
        assert mock_llm_service.llm.generate_content_async.call_count == 3

    @pytest.mark.asyncio
    async def test_concurrent_item_processing(self, item_processor, job_context, mock_llm_service):
        ...
        async def mock_generate_content(*args, **kwargs):
            ...
            return LLMResponse(
                success=True,
                content="Enhanced concurrent content",
                metadata={}
            )
        ...
        tasks = [
            item_processor.process_item(item, job_context)
            for item in items
        ]
    ...
    @pytest.mark.asyncio
    async def test_processing_statistics_integration(self, item_processor, sample_qualification_item, job_context, mock_llm_service):
...
class TestItemProcessorErrorRecovery:
    ...
    @pytest.mark.asyncio
    async def test_graceful_degradation_on_service_failure(self, item_processor, mock_llm_service):
        ...
        with patch.object(mock_llm_service, 'generate_content') as mock_generate:
            mock_generate.return_value = LLMResponse(
                success=False,
                error_message="Complete service failure"
            )
            ...
            assert result is False
            assert item.metadata.status == ProcessingStatus.FAILED
