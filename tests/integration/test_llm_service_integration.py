...
class TestLLMServiceIntegration:
            ...
            sections=[
                Section(
                    ...
                    items=[
                        ...
                        Item(id="skill_2", content="Data Analysis"),
                        Item(id="skill_3", content="Statistical Modeling"),
                    ],
                ),
                Section(
                    ...
                    items=[
                        Item(
                            id="exp_1", content="Data Analyst at DataCorp (2021-2023)"
                        ),
                        Item(
                            id="exp_2",
    ...
    @pytest.mark.asyncio
    async def test_parser_agent_llm_integration(
        self, mock_llm_service, sample_job_description
    ):
        ...
        assert (
            sample_job_description in call_args[0][0]
        )  # First positional arg (prompt)
    ...
    @pytest.mark.asyncio
    async def test_research_agent_llm_integration(
        self, mock_llm_service, sample_job_description, sample_cv_data
    ):
        ...
        def mock_llm_response(prompt, **kwargs):
            if "industry trends" in prompt.lower():
                return (
                    "AI and ML adoption is growing rapidly in enterprise environments"
                )
    ...
    @pytest.mark.asyncio
    async def test_content_writer_llm_integration(
        self, mock_llm_service, sample_cv_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_quality_assurance_llm_integration(
        self, mock_llm_service, sample_cv_data
    ):
        ...
        def mock_qa_response(prompt, **kwargs):
            if "content relevance" in prompt.lower():
                return (
                    "Score: 85. The content is highly relevant to the job requirements."
                )
    ...
    @pytest.mark.asyncio
    async def test_llm_error_handling_integration(
        self, mock_llm_service, sample_job_description
    ):
        """Test LLM service error handling across agents."""
        # Configure LLM to raise an exception
        mock_llm_service.generate_content_async.side_effect = Exception(
    ...
    @pytest.mark.asyncio
    async def test_llm_retry_mechanism_integration(
        self, mock_llm_service, sample_job_description
    ):
        ...
        mock_llm_service.generate_structured_content_async.side_effect = (
            mock_llm_with_retry
        )
    ...
    @pytest.mark.asyncio
    async def test_llm_response_validation_integration(
        self, mock_llm_service, sample_job_description
    ):
    ...
    @pytest.mark.asyncio
    async def test_llm_prompt_construction_integration(
        self, mock_llm_service, sample_job_description, sample_cv_data
    ):
        ...
        for call_args in call_args_list:
            ...
            assert any(
                ...
                for keyword in ["data scientist", "python", "machine learning"]
            )

    @pytest.mark.asyncio
    async def test_llm_concurrent_calls_integration(
        self, mock_llm_service, sample_cv_data
    ):
    ...
    def test_llm_service_configuration_integration(self, mock_llm_service):
        ...
        for agent in fallback_agents:
            assert agent.llm_service is None or hasattr(
                agent.llm_service, "generate_content_async"
            )
