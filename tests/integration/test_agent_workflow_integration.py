...
from src.orchestration.cv_workflow_graph import (
    parser_node, research_node, content_writer_node, 
    quality_assurance_node, formatter_node
)
...
class TestAgentWorkflowIntegration:
            ...
            sections=[
                ...
                CVSection(
                    ...
                    items=[
                        ...
                        CVItem(content="Developed ML models for recommendation systems"),
                        CVItem(content="Implemented data pipelines processing 1M+ records")
                    ]
                ),
                CVSection(
                    name="Education",
    ...
    async def test_parser_to_research_workflow(self, initial_state):
        ...
        with patch('src.agents.parser_agent.ParserAgent.run_as_node') as mock_parser, \
             patch('src.agents.research_agent.ResearchAgent.run_as_node') as mock_research:
    ...
    async def test_content_writer_to_qa_workflow(self, initial_state):
        ...
        with patch('src.agents.enhanced_content_writer.EnhancedContentWriterAgent.run_as_node') as mock_writer, \
             patch('src.agents.quality_assurance_agent.QualityAssuranceAgent.run_as_node') as mock_qa:
            ...
            qa_result = await quality_assurance_node(writer_result)
    ...
    async def test_qa_to_formatter_workflow(self, initial_state):
        ...
        with patch('src.agents.formatter_agent.FormatterAgent.run_as_node') as mock_formatter:
    ...
    async def test_complete_workflow_integration(self, initial_state):
        ...
        with patch('src.agents.parser_agent.ParserAgent.run_as_node') as mock_parser, \
             patch('src.agents.research_agent.ResearchAgent.run_as_node') as mock_research, \
             patch('src.agents.enhanced_content_writer.EnhancedContentWriterAgent.run_as_node') as mock_writer, \
             patch('src.agents.quality_assurance_agent.QualityAssuranceAgent.run_as_node') as mock_qa, \
             patch('src.agents.formatter_agent.FormatterAgent.run_as_node') as mock_formatter:
            ...
            result = await quality_assurance_node(result)
    ...
    def test_state_consistency_across_nodes(self, initial_state):
        ...
        required_keys = [
            "job_description_data", "structured_cv", "error_messages",
            "processing_queue", "research_data", "content_data",
            "quality_scores", "output_data"
        ]
