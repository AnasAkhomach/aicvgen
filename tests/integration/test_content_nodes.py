...
class TestContentNodesThinWrappers:
    ...
    def mock_agent(self):
        ...
        agent = AsyncMock()
        agent.run_as_node = AsyncMock(return_value={"updated": True, "last_executed_node": "TEST_NODE"})
        return agent
    ...
    async def test_professional_experience_updater_node(self, mock_state, mock_agent):
        ...
        result = await professional_experience_updater_node(mock_state, agent=mock_agent)
    ...
    async def test_qa_node(self, mock_state, mock_agent):
        ...
        mock_agent.run_as_node.return_value = {"qa_results": {"score": 0.9}, "last_executed_node": "QA"}
...
class TestContentNodesCompliance:
    ...
    def test_nodes_are_simple_wrappers(self):
        ...
        for node_func in nodes_to_check:
            ...
            assert len(code_lines) == 1, f"Node {node_func.__name__} has {len(code_lines)} lines of code, expected 1"
            assert "return await agent.run_as_node(state)" in code_lines[0], f"Node {node_func.__name__} doesn't follow thin wrapper pattern"