...
class TestSessionIdFix:
    ...
    def test_workflow_manager_creates_valid_session_id(self):
        ...
        mock_container.cv_template_loader_service.return_value = mock_cv_template_loader_service
    ...
    def test_all_agent_input_models_work_with_session_id(self):
        ...
        from src.models.cv_models import JobDescriptionData, Section, Item
        from src.orchestration.state import create_global_state
        ...
        structured_cv.sections = [
            Section(
                ...
                items=[Item(content="Python programming"), Item(content="Machine learning")]
            ),
            Section(
                name="Professional Experience", 
                items=[Item(content="Senior Developer at TechCorp"), Item(content="Led team of 5 developers")]
            ),
            Section(
                name="Project Experience",
                items=[Item(content="Built AI-powered CV generator"), Item(content="Developed REST API")]
            )
        ]
        ...
        for agent_name, model_class in AGENT_INPUT_MODELS.items():
            ...
            try:
                ...
                if hasattr(model_class, 'model_fields') and 'session_id' in model_class.model_fields:
                    assert hasattr(model_instance, 'session_id'), f"Model {agent_name} should have session_id attribute"
                    assert model_instance.session_id == state["session_id"], f"Model {agent_name} session_id should be {state['session_id']}"
                else:
                    print(f"Model {agent_name} does not have session_id field, skipping session_id check")
