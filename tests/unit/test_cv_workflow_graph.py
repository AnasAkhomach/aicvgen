...
class TestWorkflowGraphModular:
    ...
    def sample_state(self):
        ...
        section1 = Section(name="Key Qualifications", items=[item1])
    ...
    def test_build_main_workflow_graph(self, mock_container):
        ...
        with patch(
            "src.orchestration.graphs.main_graph.build_key_qualifications_subgraph"
        ) as mock_kq, patch(
            "src.orchestration.graphs.main_graph.build_professional_experience_subgraph"
        ) as mock_pe, patch(
            "src.orchestration.graphs.main_graph.build_projects_subgraph"
        ) as mock_proj, patch(
            "src.orchestration.graphs.main_graph.build_executive_summary_subgraph"
        ) as mock_exec:
    ...
    def test_create_node_functions(self, mock_container):
        ...
        # Verify all expected node functions are created (MVP implementation)
        expected_functions = [
            "user_cv_parser_node",
            "jd_parser_node",
        ...
        # Verify deferred nodes are not included in MVP
        deferred_functions = [
            "research_node",
            "cv_analyzer_node",
            "qa_node",
        ...
        for func_name in deferred_functions:
            assert func_name not in node_functions

    @pytest.mark.asyncio
    async def test_handle_feedback_node_with_valid_feedback(
        self, mock_container, sample_state
    ):
    ...
    @pytest.mark.asyncio
    async def test_handle_feedback_node_with_missing_item_id(
        self, mock_container, sample_state
    ):
    ...
    @pytest.mark.asyncio
    async def test_handle_feedback_node_with_none_item_id(
        self, mock_container, sample_state
    ):
    ...
    @pytest.mark.asyncio
    async def test_handle_feedback_node_without_feedback(
        self, mock_container, sample_state
    ):
    ...
    @pytest.mark.asyncio
    async def test_handle_feedback_node_raises_value_error_for_invalid_item_id(
        self, mock_container, sample_state
    ):
