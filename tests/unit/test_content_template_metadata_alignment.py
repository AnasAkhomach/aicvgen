...
class TestContentTemplateMetadataFix:
    ...
    def test_professional_experience_prompt_content_type(self):
        ...
        assert "content_type: EXPERIENCE" in content, "content_type should be EXPERIENCE"
        
        # Verify EXPERIENCE is a valid ContentType enum member
        assert hasattr(ContentType, 'EXPERIENCE'), "EXPERIENCE should be a ContentType enum member"
    ...
    def test_projects_prompt_content_type(self):
        ...
        assert hasattr(ContentType, 'PROJECT'), "PROJECT should be a ContentType enum member"
    ...
    def test_content_template_manager_can_load_templates(self):
        ...
        experience_templates = template_manager.get_templates_by_type(ContentType.EXPERIENCE)
        assert len(experience_templates) > 0, "Should find at least one EXPERIENCE template"
    ...
    def test_template_metadata_parsing(self):
        ...
        for template in experience_templates:
            if "resume_role" in template.name or "professional_experience" in template.name:
                professional_exp_template = template
                break
        
        assert professional_exp_template is not None, "Should find professional experience template"
    ...
    def test_no_old_content_types_remain(self):
        ...
        assert "content_type: role_generation" not in prof_exp_content, \
        ...
        assert "content_type: project_generation" not in projects_content, \
    ...
    def test_content_type_enum_completeness(self):
        ...
        for content_type_name in required_content_types:
            assert hasattr(ContentType, content_type_name), \
