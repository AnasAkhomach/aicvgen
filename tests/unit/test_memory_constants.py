...
class TestMemoryConstants:
    ...
    def test_memory_constants_values(self):
        ...
        assert (
            MemoryConstants.MEMORY_CRITICAL_THRESHOLD
            > MemoryConstants.MEMORY_WARNING_THRESHOLD
        )
        ...
        for attr_name in dir(MemoryConstants):
            if not attr_name.startswith("_"):
                ...
                if isinstance(value, (int, float)):
                    assert (
                        value not in obviously_hardcoded_values
                    ), f"Memory constant {value} appears to be an obviously hardcoded value. Consider using a more specific value."
