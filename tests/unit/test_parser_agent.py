...
class TestParserAgent:
    ...
    async def test_run_as_node_success(self, parser_agent, sample_agent_state):
        ...
        with patch.object(parser_agent, 'parse_job_description') as mock_parse:
            ...
            assert result_state["job_description_data"]["job_title"] == "Senior Software Engineer"
            assert "Python" in result_state["job_description_data"]["required_skills"]
            assert "error_messages" not in result_state or len(result_state["error_messages"]) == 0
            mock_parse.assert_called_once_with(sample_agent_state.job_description_data.raw_text)
    ...
    async def test_run_as_node_empty_job_description(self, parser_agent):
        ...
        assert "job_description_data" in result_state or "error_messages" in result_state
    ...
    async def test_run_as_node_parsing_error(self, parser_agent, sample_agent_state):
        ...
        with patch.object(parser_agent, 'parse_job_description') as mock_parse:
            ...
            assert "ParserAgent Error: Parsing failed" in result_state["error_messages"][0]
    ...
    async def test_run_as_node_with_cv_text(self, parser_agent):
        ...
        with patch.object(parser_agent, 'parse_job_description') as mock_parse_job, \
             patch.object(parser_agent, 'parse_cv_text') as mock_parse_cv:
            ...
            mock_parse_cv.assert_called_once_with("Sample CV text content", result["job_description_data"])
    ...
    async def test_run_as_node_start_from_scratch(self, parser_agent):
        ...
        with patch.object(parser_agent, 'parse_job_description') as mock_parse_job, \
             patch.object(parser_agent, 'create_empty_cv_structure') as mock_create_cv:
    ...
    async def test_run_as_node_error_handling(self, parser_agent):
        ...
        with patch.object(parser_agent, 'parse_job_description') as mock_parse:
            ...
            assert any("ParserAgent Error: Test error" in msg for msg in result["error_messages"])
