...
class TestErrorRecoveryService:
    ...
    def sample_item(self):
        ...
        return Item(
            ...
            metadata=ItemMetadata(
                status=ProcessingStatus.IN_PROGRESS, created_at=datetime.now()
            ),
        )
    ...
    def test_error_classification_by_type(self, error_recovery_service):
        ...
        assert (
            error_recovery_service.classify_error(validation_error)
        ...
        assert (
            error_recovery_service.classify_error(parsing_error)
        ...
        assert (
            error_recovery_service.classify_error(rate_limit_error)
        ...
        assert (
            error_recovery_service.classify_error(network_error)
        ...
        assert (
            error_recovery_service.classify_error(timeout_error)
    ...
    def test_error_classification_by_message(self, error_recovery_service):
        ...
        assert (
            error_recovery_service.classify_error(generic_error) == ErrorType.RATE_LIMIT
        )
        ...
        assert (
            error_recovery_service.classify_error(unknown_error)
            == ErrorType.UNKNOWN_ERROR
        )

    def test_recovery_strategy_determination(
        self, error_recovery_service, sample_error_context
    ):
    ...
    def test_max_retries_exceeded_fallback(
        self, error_recovery_service, sample_error_context
    ):
    ...
    def test_circuit_breaker_functionality(
        ...
        should_break = error_recovery_service._should_circuit_break(
            ErrorType.SYSTEM_ERROR, sample_error_context.session_id
        )
    ...
    def test_fallback_content_generation(
        self, error_recovery_service, sample_error_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_execute_recovery_action_fallback_content(
        self, error_recovery_service, sample_item
    ):
    ...
    @pytest.mark.asyncio
    async def test_execute_recovery_action_skip_item(
        self, error_recovery_service, sample_item
    ):
        """Test execution of skip item recovery action."""
        action = RecoveryAction(
            strategy=RecoveryStrategy.SKIP_ITEM, should_continue=True
        )
    ...
    @pytest.mark.asyncio
    async def test_execute_recovery_action_circuit_breaker(
        self, error_recovery_service, sample_item
    ):
        """Test execution of circuit breaker recovery action."""
        action = RecoveryAction(
            strategy=RecoveryStrategy.CIRCUIT_BREAKER, should_continue=False
        )
    ...
    def test_error_summary_generation(
        ...
        # Generate summary
        summary = error_recovery_service.get_error_summary(
    ...
    def test_delay_calculation_exponential_backoff(
        self, error_recovery_service, sample_error_context
    ):
        ...
        delay_0 = error_recovery_service._calculate_delay(
            base_action, 0, sample_error_context
        )
        delay_1 = error_recovery_service._calculate_delay(
            base_action, 1, sample_error_context
        )
        delay_2 = error_recovery_service._calculate_delay(
            base_action, 2, sample_error_context
        )
    ...
    def test_delay_calculation_linear_backoff(
        self, error_recovery_service, sample_error_context
    ):
        """Test delay calculation for linear backoff strategy."""
        base_action = RecoveryAction(
            strategy=RecoveryStrategy.LINEAR_BACKOFF, delay_seconds=5.0, max_retries=3
        )
    ...
    def test_retry_after_extraction_from_message(
        self, error_recovery_service, sample_error_context
    ):
        """Test extraction of retry-after value from error message."""
        # Test retry-after pattern
        sample_error_context.error_message = (
            "Rate limit exceeded. Retry after 120 seconds."
        )
        retry_after = error_recovery_service._extract_retry_after_from_message(
    ...
    def test_retry_after_extraction_from_metadata(
        self, error_recovery_service, sample_error_context
    ):
