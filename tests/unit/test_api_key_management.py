...
class TestAPIKeyManagement:
    ...
    def test_determine_active_api_key_fallback_only(self, mock_get_config, mock_genai):
        ...
        assert service.using_user_key is False
        assert (
            service.using_fallback is False
        )  # Not switched to fallback, using it as primary

        # Verify API key info
        info = service.get_current_api_key_info()
        assert (
            info["key_source"] == "primary"
        )  # Using fallback as primary since no primary available
