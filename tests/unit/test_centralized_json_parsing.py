...
class TestAgent(EnhancedAgentBase):
    ...
    def __init__(self, llm_service=None):
        ...
        self.llm = llm_service  # Set the llm attribute that _generate_and_parse_json looks for
...
class TestCentralizedJsonParsing:
    ...
    @pytest.mark.asyncio
    async def test_base_agent_generate_and_parse_json_success(self, mock_llm_service, sample_markdown_response, sample_json_response):
    ...
    async def test_parser_agent_uses_centralized_method(self, mock_get_llm, mock_llm_service, agent_state):
        ...
        with patch.object(agent, '_generate_and_parse_json', new_callable=AsyncMock) as mock_centralized:
            mock_centralized.return_value = {"skills": ["Python"], "experience_level": "Senior"}
    ...
    async def test_research_agent_uses_centralized_method(self, mock_get_llm, mock_llm_service, agent_state):
        ...
        # Mock the centralized method to verify it's called
        with patch.object(agent, '_generate_and_parse_json', new_callable=AsyncMock) as mock_centralized:
            mock_centralized.return_value = {"company_name": "Test Corp", "industry": "Tech"}
