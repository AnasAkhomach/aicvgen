...
from src.models.data_models import Item, ProcessingStatus, ItemStatus, LLMResponse
...
class TestItemProcessorSimplified:
    ...
    @pytest.mark.asyncio
    async def test_successful_item_processing(
        self, item_processor, sample_item, job_context, mock_llm_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_failed_item_processing_no_retries(
        self, item_processor, sample_item, job_context, mock_llm_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_item_not_pending_status(
        self, item_processor, sample_item, job_context, mock_llm_service
    ):
    ...
    async def test_configuration_error_propagation(
        ...
        mock_llm_service.generate_content.side_effect = ConfigurationError(
            "Invalid API key"
        )
    ...
    @pytest.mark.asyncio
    async def test_qualification_content_generation(
        ...
        mock_response = LLMResponse(
            success=True, content="Enhanced Python qualification", metadata={}
        )
        mock_llm_service.generate_content.return_value = mock_response
    ...
    @pytest.mark.asyncio
    async def test_experience_content_generation(
        self, item_processor, mock_llm_service
    ):
        ...
        job_context = {"job_title": "Senior Developer", "company": "NewCorp"}

        # Mock successful response
        mock_response = LLMResponse(
            success=True, content="Enhanced experience description", metadata={}
        )
        mock_llm_service.generate_content.return_value = mock_response
    ...
    async def test_project_content_generation(self, item_processor, mock_llm_service):
        ...
        # Mock successful response
        mock_response = LLMResponse(
            success=True, content="Enhanced project description", metadata={}
        )
        mock_llm_service.generate_content.return_value = mock_response
    ...
    def test_qa_callback_integration(self):
        ...
        processor = ItemProcessor(
            llm_client=mock_llm_service, qa_callback=mock_qa_callback
        )
...
class TestItemProcessorMetrics:
    ...
    @pytest.mark.asyncio
    async def test_processing_statistics_success(
        self, item_processor, mock_llm_service
    ):
        ...
        item = Item(
            ...
            status=ItemStatus.INITIAL,
            metadata={"item_id": "test-1", "item_type": "qualification"},
        )
    ...
    @pytest.mark.asyncio
    async def test_processing_statistics_failure(
        self, item_processor, mock_llm_service
    ):
