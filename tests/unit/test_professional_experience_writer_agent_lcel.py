...
class TestProfessionalExperienceWriterAgentLCEL:
    ...
    def mock_prompt(self):
        ...
        mock_prompt.input_variables = ['job_title', 'company_name', 'job_description', 'experience_item', 'cv_summary', 'required_skills', 'preferred_qualifications', 'research_findings']
        # Make the mock support the pipe operator
    ...
    def agent(self, mock_llm, mock_prompt, mock_parser, mock_chain):
        ...
        with patch.object(ProfessionalExperienceWriterAgent, '__init__', return_value=None):
            agent = ProfessionalExperienceWriterAgent.__new__(ProfessionalExperienceWriterAgent)
    ...
    async def test_execute_with_valid_input(self, agent, sample_input_data):
        ...
        assert isinstance(result["generated_professional_experience"], ProfessionalExperienceLLMOutput)
        assert result["generated_professional_experience"].professional_experience == "Generated professional experience content for the role."
    ...
    def test_pydantic_output_model_validation(self):
        ...
        assert output.professional_experience == valid_data["professional_experience"].strip()

        # Invalid data - too short (Pydantic's built-in validation message)
        with pytest.raises(Exception, match="String should have at least 50 characters"):
