...
class TestEnhancedAgentBase:
    ...
    def test_concrete_class_must_implement_run_as_node(self):
        ...
        # Act & Assert
        with pytest.raises(
            TypeError, match="Can't instantiate abstract class.*run_as_node"
        ):
            IncompleteAgent(
                name="incomplete", description="test", llm_service=MagicMock()
            )
...
class TestAgentInterfaceCompliance:
    ...
    def test_qa_agent_compliance(self):
        ...
        agent = QualityAssuranceAgent(
            name="qa", description="test", llm_service=mock_llm
        )
