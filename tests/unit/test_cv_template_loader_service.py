...
"""
        ...
        try:
            ...
            experience_section = next(s for s in result.sections if s.name == "Experience")
            ...
            education_section = next(s for s in result.sections if s.name == "Education")
...
"""
    ...
    def test_pydantic_validation_error(self, mock_structured_cv):
        ...
        mock_structured_cv.side_effect = ValidationError.from_exception_data(
            'StructuredCV', [{'type': 'missing', 'loc': ('test_field',), 'msg': 'Field required'}]
        )
