...
class TestMemoryOptimizer:
    ...
    @patch('src.utils.performance.psutil.Process')
    @patch('src.utils.performance.psutil.virtual_memory')
    def test_should_run_gc_memory_threshold(self, mock_virtual_memory, mock_process, optimizer):
        """Test GC trigger based on memory threshold."""
        mock_process.return_value.memory_info.return_value.rss = 150 * 1024 * 1024  # 150MB > threshold
    ...
    @patch('src.utils.performance.psutil.Process')
    @patch('src.utils.performance.psutil.virtual_memory')
    def test_should_run_gc_time_threshold(self, mock_virtual_memory, mock_process, optimizer):
        """Test GC trigger based on time threshold."""
        mock_process.return_value.memory_info.return_value.rss = 50 * 1024 * 1024  # 50MB < threshold
    ...
    @patch('src.utils.performance.psutil.Process')
    @patch('src.utils.performance.psutil.virtual_memory')
    def test_optimize_memory(self, mock_virtual_memory, mock_process, mock_gc_collect, optimizer):
        """Test memory optimization."""
        # Mock memory before optimization
        mock_process.return_value.memory_info.return_value.rss = 150 * 1024 * 1024  # 150MB
        mock_process.return_value.memory_info.return_value.vms = 200 * 1024 * 1024
        mock_process.return_value.memory_percent.return_value = 7.5
        mock_virtual_memory.return_value.available = 1000 * 1024 * 1024
        mock_virtual_memory.return_value.total = 2000 * 1024 * 1024
        ...
        def memory_side_effect():
            if mock_gc_collect.called:
                mock_process.return_value.memory_info.return_value.rss = 140 * 1024 * 1024  # 140MB
...
class TestDecorators:
    ...
    def test_auto_memory_optimize_sync(self, mock_get_optimizer):
        ...
        mock_optimizer = Mock()
        mock_optimizer.get_memory_info.return_value = {"rss_mb": 150.0}  # Above threshold
