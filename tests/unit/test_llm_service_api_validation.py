...
class TestEnhancedLLMServiceAPIValidation:
    ...
    async def test_validate_api_key_success(self, llm_service):
        ...
        mock_models = [
            Mock(name="gemini-pro"),
            Mock(name="gemini-pro-vision")
        ]
        
        with patch('src.services.llm_service.genai.list_models', return_value=mock_models):
    ...
    async def test_validate_api_key_authentication_failure(self, llm_service):
        ...
        with patch('src.services.llm_service.genai.list_models', side_effect=Exception("Invalid API key")):
            result = await llm_service.validate_api_key()
    ...
    async def test_validate_api_key_network_error(self, llm_service):
        ...
        with patch('src.services.llm_service.genai.list_models', side_effect=ConnectionError("Network unreachable")):
            result = await llm_service.validate_api_key()
    ...
    async def test_validate_api_key_generic_exception(self, llm_service):
        ...
        with patch('src.services.llm_service.genai.list_models', side_effect=RuntimeError("Unexpected error")):
            result = await llm_service.validate_api_key()
    ...
    def test_service_initialization_without_api_key_fails_fast(self, mock_genai_configure, mock_genai_model):
        ...
        with patch('src.services.llm_service.get_config') as mock_config:
            ...
            with pytest.raises(ConfigurationError, match="CRITICAL: Gemini API key is not configured"):
    ...
    async def test_validate_api_key_uses_executor(self, llm_service):
        ...
        with patch('src.services.llm_service.genai.list_models', return_value=mock_models) as mock_list_models:
