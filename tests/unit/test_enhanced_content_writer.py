...
class TestEnhancedContentWriterAgent:
    ...
    async def test_run_async_success(
        ...
        with patch.object(
            content_writer_agent, "_enhance_experience_items"
        ) as mock_enhance_exp, patch.object(
            content_writer_agent, "_enhance_skills_items"
        ) as mock_enhance_skills, patch.object(
            content_writer_agent, "_enhance_education_items"
        ) as mock_enhance_edu:
            ...
            result = await content_writer_agent.run_async(
                content_data, execution_context
            )
    ...
    async def test_run_async_missing_structured_cv(
        ...
        content_data = {"job_data": {"title": "Engineer"}, "research_findings": {}}
    ...
    async def test_run_async_missing_job_data(
        ...
        content_data = {"structured_cv": sample_structured_cv, "research_findings": {}}
    ...
    async def test_process_single_item_success(
        ...
        with patch.object(content_writer_agent, "_enhance_single_item") as mock_enhance:
            mock_enhance.return_value = (
                "Enhanced: Software Engineer at TechCorp with 5+ years experience"
            )

            result_cv = await content_writer_agent._process_single_item(
                sample_structured_cv, item_id, sample_research_findings
            )
    ...
    async def test_enhance_single_item_experience(
        self, content_writer_agent, mock_llm_service
    ):
        ...
        enhanced_content = await content_writer_agent._enhance_single_item(
            item, "Professional Experience", research_findings
        )
    ...
    async def test_enhance_single_item_skills(
        self, content_writer_agent, mock_llm_service
    ):
        """Test enhancing a single skills item."""
        item = Item(
            id="skill_1", content="Python programming", status=ItemStatus.PENDING
        )
        ...
        enhanced_content = await content_writer_agent._enhance_single_item(
            item, "Technical Skills", research_findings
        )
    ...
    async def test_enhance_experience_items(
        self, content_writer_agent, sample_structured_cv, sample_research_findings
    ):
    ...
    async def test_enhance_education_items(
        self, content_writer_agent, sample_structured_cv, sample_research_findings
    ):
    ...
    async def test_generate_big_10_skills_success(
        ...
        # Mock prompt loading
        with patch.object(
            content_writer_agent, "_load_prompt_template"
        ) as mock_load_prompt:
            ...
            skills = await content_writer_agent.generate_big_10_skills(
                sample_job_data, sample_research_findings
            )
    ...
    def test_build_enhancement_prompt(self, content_writer_agent):
        ...
        prompt = content_writer_agent._build_enhancement_prompt(
            item, section_name, research_findings
        )
    ...
    def test_extract_relevant_research(self, content_writer_agent):
        ...
        relevant = content_writer_agent._extract_relevant_research(
            "Professional Experience", research_findings
        )
        ...
        relevant = content_writer_agent._extract_relevant_research(
            "Technical Skills", research_findings
        )
    ...
    def test_confidence_score_calculation(self, content_writer_agent):
        ...
        minimal_output = {"structured_cv": StructuredCV(), "items_enhanced": 1}
    ...
    def test_find_item_by_id(self, content_writer_agent, sample_structured_cv):
        ...
        item, section = content_writer_agent._find_item_by_id(
            sample_structured_cv, "exp_1"
        )
    ...
    def test_find_item_by_id_not_found(
        self, content_writer_agent, sample_structured_cv
    ):
        """Test finding non-existent item by ID."""
        item, section = content_writer_agent._find_item_by_id(
            sample_structured_cv, "nonexistent"
        )

        assert item is None
        assert section is None

    def test_get_section_items_by_name(
        self, content_writer_agent, sample_structured_cv
    ):
        """Test getting section items by name."""
        items = content_writer_agent._get_section_items_by_name(
            sample_structured_cv, "Professional Experience"
        )
    ...
    def test_get_section_items_by_name_not_found(
        self, content_writer_agent, sample_structured_cv
    ):
        """Test getting items from non-existent section."""
        items = content_writer_agent._get_section_items_by_name(
            sample_structured_cv, "Nonexistent Section"
        )
    ...
    async def test_run_as_node_logs_execution(
        self, mock_logger, content_writer_agent, sample_agent_state
    ):
    ...
    def test_validate_enhancement_quality(self, content_writer_agent):
        ...
        is_valid = content_writer_agent._validate_enhancement_quality(
            original_content, poor_enhancement
        )
