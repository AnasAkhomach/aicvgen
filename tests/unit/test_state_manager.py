...
class TestStateManager:
    ...
    async def test_update_state_field(self, state_manager, sample_agent_state):
        ...
        await state_manager.update_state_field(
            session_id, "current_item_id", new_item_id
        )
    ...
    async def test_update_state_field_invalid_session(self, state_manager):
        ...
        with pytest.raises(StateManagerError, match="Session not found"):
            await state_manager.update_state_field(
                "invalid_session", "current_item_id", "new_value"
            )

    @pytest.mark.asyncio
    async def test_update_state_field_invalid_field(
        self, state_manager, sample_agent_state
    ):
        ...
        with pytest.raises(StateManagerError, match="Invalid field"):
            await state_manager.update_state_field(
                session_id, "invalid_field", "new_value"
            )
    ...
    @pytest.mark.asyncio
    async def test_remove_from_processing_queue(
        self, state_manager, sample_agent_state
    ):
    ...
    async def test_state_history_tracking(self, state_manager, sample_agent_state):
        ...
        for i in range(3):
            await state_manager.update_state_field(
                session_id, "current_item_id", f"item_{i}"
            )
    ...
    async def test_concurrent_state_operations(self, state_manager, sample_agent_state):
        ...
        async def update_state(field_value):
            await state_manager.update_state_field(
                session_id, "current_item_id", f"item_{field_value}"
            )
    ...
    @pytest.mark.asyncio
    async def test_state_persistence_across_instances(
        self, temp_state_dir, sample_agent_state
    ):
    ...
    @pytest.mark.asyncio
    async def test_export_state(
        self, state_manager, sample_agent_state, temp_state_dir
    ):
