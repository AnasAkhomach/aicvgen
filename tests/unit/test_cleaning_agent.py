...
class TestCleaningAgentErrorHandling:
    ...
    async def test_error_handler_call_fix(self, cleaning_agent, mock_context):
        ...
        with patch.object(AgentErrorHandler, 'handle_general_error') as mock_handler:
            ...
            with patch.object(cleaning_agent, '_clean_big_10_skills', side_effect=Exception("Test error")):
    ...
    @pytest.mark.asyncio
    async def test_no_handle_agent_error_method_called(self, cleaning_agent, mock_context):
    ...
    @pytest.mark.asyncio
    async def test_successful_execution_no_error_handler(self, cleaning_agent, mock_context):
        """Test that error handler is not called on successful execution."""
        with patch.object(AgentErrorHandler, 'handle_general_error') as mock_handler:
            with patch.object(cleaning_agent, '_clean_big_10_skills', return_value=["Python", "JavaScript"]):
                result = await cleaning_agent.run_async(mock_context)
