...
class TestCleaningAgent:
    ...
    def test_initialization(self, cleaning_agent):
        ...
        assert (
            cleaning_agent.description
            == "Processes and cleans raw LLM outputs into structured data"
        )
        assert cleaning_agent.content_type == ContentType.SKILLS
        assert cleaning_agent.input_schema.required_fields == [
            "raw_output",
            "output_type",
        ]
        assert cleaning_agent.output_schema.required_fields == [
            "cleaned_data",
            "confidence_score",
        ]

    @pytest.mark.asyncio
    async def test_clean_big_10_skills_numbered_list(
        self, cleaning_agent, execution_context
    ):
        ...
        input_data = {"raw_output": raw_output, "output_type": "big_10_skills"}
    ...
    @pytest.mark.asyncio
    async def test_clean_big_10_skills_bullet_points(
        self, cleaning_agent, execution_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_clean_big_10_skills_json_format(
        self, cleaning_agent, execution_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_clean_big_10_skills_comma_separated(
        self, cleaning_agent, execution_context
    ):
    ...
    async def test_clean_content_item(self, cleaning_agent, execution_context):
        ...
        **This is bold text** and *this is italic*. Here's some `code` formatting.
        ...
        input_data = {"raw_output": raw_output, "output_type": "content_item"}
    ...
    async def test_clean_generic_output(self, cleaning_agent, execution_context):
        ...
        input_data = {"raw_output": raw_output, "output_type": "generic"}
    ...
    async def test_confidence_score_calculation(
        ...
        poor_input = {"raw_output": "a\nb\nc", "output_type": "big_10_skills"}
