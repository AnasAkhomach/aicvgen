...
class TestCVAgentManagerLifecycle:
    ...
    def test_initialization_success(
        self, mock_error_handler, mock_get_lifecycle_manager
    ):
    ...
    def test_get_agent_success(self, mock_error_handler, mock_get_lifecycle_manager):
        ...
        mock_lifecycle_manager.get_agent.assert_called_once_with(
            agent_type="test_agent", session_id=self.session_id
        )

    @patch("src.integration.cv_agent_manager.get_agent_lifecycle_manager")
    @patch("src.integration.cv_agent_manager.AgentErrorHandler")
    def test_get_agent_with_session_override(
        self, mock_error_handler, mock_get_lifecycle_manager
    ):
        ...
        assert result == mock_agent
        mock_lifecycle_manager.get_agent.assert_called_once_with(
            agent_type="test_agent", session_id=override_session
        )

    @patch("src.integration.cv_agent_manager.get_agent_lifecycle_manager")
    @patch("src.integration.cv_agent_manager.AgentErrorHandler")
    def test_get_agent_error_handling(
        self, mock_error_handler, mock_get_lifecycle_manager
    ):
        """Test error handling during agent retrieval."""
        # Setup mocks
        mock_lifecycle_manager = Mock()
        mock_lifecycle_manager.get_agent.side_effect = AgentExecutionError(
        ...
        mock_error_handler_instance.handle_general_error.return_value = (
            mock_fallback_agent
        )
    ...
    def test_list_agents_success(self, mock_error_handler, mock_get_lifecycle_manager):
        ...
        setattr(
            mock_container, "some_service", not_an_agent_mock
        )  # This doesn't end with _agent
    ...
    def test_uninitialized_operations(self):
        ...
        with patch(
            "src.integration.cv_agent_manager.get_agent_lifecycle_manager"
        ) as mock_get_lifecycle_manager:
            mock_get_lifecycle_manager.return_value = None
