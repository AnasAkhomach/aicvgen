...
class TestCVAnalyzerAgent:
    ...
    def sample_structured_cv(self):
        ...
        cv.sections = [
            ...
            Section(
                ...
                items=[
                    Item(
                        content="Software Engineer at TechCorp (2020-2024)",
                    ...
                    Item(
                        content="Led team of 3 developers", status=ItemStatus.GENERATED
                    ),
                ],
            ),
            Section(
                ...
                items=[
                    Item(
                        content="Python, JavaScript, React, Django",
                        status=ItemStatus.GENERATED,
                    ),
                    Item(
                        content="AWS, Docker, Kubernetes", status=ItemStatus.GENERATED
                    ),
                ],
            ),
            Section(
                ...
                items=[
                    Item(
                        content="BS Computer Science, MIT (2016-2020)",
                        status=ItemStatus.GENERATED,
                    )
                ],
            ),
    ...
    def test_cv_analyzer_initialization_without_llm(self):
        ...
        agent = CVAnalyzerAgent(name="TestCVAnalyzer", description="Test description")
    ...
    async def test_run_async_success(
        ...
        with patch.object(
            cv_analyzer_agent, "_analyze_content_relevance"
        ) as mock_relevance, patch.object(
            cv_analyzer_agent, "_analyze_skill_alignment"
        ) as mock_skills, patch.object(
            cv_analyzer_agent, "_analyze_experience_match"
        ) as mock_experience, patch.object(
            cv_analyzer_agent, "_generate_recommendations"
        ) as mock_recommendations:

            mock_relevance.return_value = {
                "score": 0.85,
                "details": "Good content relevance",
            }
    ...
    async def test_run_async_missing_structured_cv(
        self, cv_analyzer_agent, execution_context
    ):
        """Test run_async with missing structured CV."""
        content_data = {"job_data": {"title": "Engineer"}}
    ...
    async def test_run_async_missing_job_data(
        self, cv_analyzer_agent, sample_structured_cv, execution_context
    ):
        """Test run_async with missing job data."""
        content_data = {"structured_cv": sample_structured_cv}
    ...
    async def test_run_as_node_missing_structured_cv(
        self, cv_analyzer_agent, sample_job_data
    ):
    ...
    async def test_run_as_node_missing_job_data(
        self, cv_analyzer_agent, sample_structured_cv
    ):
    ...
    async def test_run_as_node_analysis_error(
        self, cv_analyzer_agent, sample_agent_state
    ):
        ...
        with patch.object(cv_analyzer_agent, "run_async") as mock_run_async:
            ...
            assert len(result_state.error_messages) == 1
            assert (
                "Error during CV analysis: Analysis failed"
                in result_state.error_messages[0]
            )
            assert result_state.analysis_results is None

    async def test_analyze_content_relevance_high_score(
        self, cv_analyzer_agent, sample_structured_cv, sample_job_data, mock_llm_service
    ):
    ...
    async def test_analyze_content_relevance_low_score(
        self, cv_analyzer_agent, sample_structured_cv, sample_job_data, mock_llm_service
    ):
    ...
    async def test_analyze_skill_alignment_good_match(
        self, cv_analyzer_agent, sample_structured_cv, sample_job_data
    ):
    ...
    async def test_analyze_skill_alignment_poor_match(
        self, cv_analyzer_agent, sample_job_data
    ):
        ...
        poor_cv.sections = [
            Section(
                ...
                items=[
                    Item(
                        content="COBOL, FORTRAN, Assembly", status=ItemStatus.GENERATED
                    )
                ],
            )
        ]
    ...
    async def test_analyze_experience_match_good_match(
        self, cv_analyzer_agent, sample_structured_cv, sample_job_data, mock_llm_service
    ):
    ...
    async def test_analyze_experience_match_poor_match(
        self, cv_analyzer_agent, sample_structured_cv, sample_job_data, mock_llm_service
    ):
    ...
    async def test_generate_recommendations_comprehensive(
        self, cv_analyzer_agent, mock_llm_service
    ):
        ...
        recommendations = await cv_analyzer_agent._generate_recommendations(
            analysis_results, {"title": "Senior Software Engineer"}
        )
    ...
    async def test_generate_recommendations_minimal(
        ...
        recommendations = await cv_analyzer_agent._generate_recommendations(
            analysis_results, {"title": "Software Engineer"}
        )
    ...
    def test_extract_cv_skills_no_skills_section(self, cv_analyzer_agent):
        ...
        cv.sections = [
            Section(
                name="Education",
                items=[
                    Item(content="BS Computer Science", status=ItemStatus.GENERATED)
                ],
            )
        ]
    ...
    def test_confidence_score_calculation(self, cv_analyzer_agent):
        ...
        minimal_output = {"analysis_results": {"overall_score": 0.45}}
    ...
    def test_build_analysis_prompt(
        ...
        prompt = cv_analyzer_agent._build_analysis_prompt(
            sample_structured_cv, sample_job_data, "content_relevance"
        )
    ...
    async def test_run_as_node_logs_execution(
        self, mock_logger, cv_analyzer_agent, sample_agent_state
    ):
        ...
        with patch.object(cv_analyzer_agent, "run_async") as mock_run_async:
            ...
            assert any(
                "Starting CV analysis" in str(call)
