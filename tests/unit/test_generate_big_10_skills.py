...
class TestGenerateBig10Skills:
    ...
    @patch(
        "src.agents.enhanced_content_writer.EnhancedContentWriterAgent._load_prompt_template"
    )
    ...
    def test_generate_big_10_skills_success(self, mock_llm_service, mock_load_prompt):
        ...
        mock_load_prompt.return_value = (
            "Generate skills based on: {job_description} and {my_talents}"
        )
        ...
        result = self.agent.generate_big_10_skills(
            job_description=self.sample_job_description, my_talents=self.sample_talents
        )
    ...
    @patch("src.agents.enhanced_content_writer.get_llm_service")
    def test_generate_big_10_skills_llm_failure(
        self, mock_llm_service, mock_load_prompt
    ):
