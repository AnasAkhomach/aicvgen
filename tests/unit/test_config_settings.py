...
class TestLLMSettings:
    ...
    def test_llm_settings_custom_values(self):
        ...
        settings = LLMSettings(
            default_model="gemini-1.5-pro", default_temperature=0.5, max_tokens=2000
        )
...
class TestAppConfigIntegration:
    ...
    def test_get_prompt_path_by_key_success(self):
        ...
        with patch("os.path.exists", return_value=True):
            ...
            expected_path = os.path.join(
                str(config.prompts_directory), expected_filename
            )
    ...
    def test_get_prompt_path_by_key_invalid_key(self):
        ...
        with pytest.raises(
            ValueError, match="Prompt key 'invalid_key' not found in settings"
        ):
    ...
    def test_get_prompt_path_by_key_file_not_exists(self):
        ...
        with patch("os.path.exists", return_value=False):
            with pytest.raises(
                FileNotFoundError, match="Prompt file does not exist at path"
            ):
