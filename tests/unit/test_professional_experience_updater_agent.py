...
class TestProfessionalExperienceUpdaterAgent:
    ...
    def updater_agent(self):
        ...
        return ProfessionalExperienceUpdaterAgent(
            ...
            name="test_professional_experience_updater"
        )
    ...
    def test_input_validation_missing_structured_cv(self, updater_agent, generated_experience_data):
        ...
        with pytest.raises(AgentExecutionError, match="Missing required input: structured_cv"):
    ...
    def test_input_validation_missing_generated_data(self, updater_agent, mock_structured_cv):
        ...
        with pytest.raises(AgentExecutionError, match="Missing required input: generated_professional_experience"):
            updater_agent._validate_inputs(invalid_input)

    def test_input_validation_empty_generated_data(self, updater_agent, mock_structured_cv):
        ...
        with pytest.raises(AgentExecutionError, match="generated_professional_experience cannot be empty"):
            updater_agent._validate_inputs(invalid_input)
    ...
    @pytest.mark.asyncio
    async def test_execute_with_existing_section(self, updater_agent, agent_input, generated_experience_data):
    ...
    @pytest.mark.asyncio
    async def test_execute_without_existing_section(self, updater_agent, generated_experience_data):
        ...
        assert "Professional Experience section not found" in result["error_messages"][0]
    ...
    async def test_execute_with_empty_generated_data(self, updater_agent, mock_structured_cv):
        ...
        result = await updater_agent.run(**agent_input.model_dump())
        assert "error_messages" in result
        assert "generated_professional_experience cannot be empty" in result["error_messages"][0]

    @patch('src.agents.professional_experience_updater_agent.logger')
    @pytest.mark.asyncio
    async def test_logging_during_execution(self, mock_logger, updater_agent, agent_input):
        """Test that appropriate logging occurs during execution."""
        await updater_agent.run(**agent_input.model_dump())
        
        # Verify logging calls
        mock_logger.info.assert_called()
        assert any("Updated Professional Experience section with generated content" in str(call) 
