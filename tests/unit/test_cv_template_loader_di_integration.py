...
class TestCVTemplateLoaderDIIntegration:
    ...
    def test_workflow_manager_receives_cv_template_loader_service(self):
        ...
        # Create WorkflowManager directly with container (as done in the actual code)
        ...
        assert isinstance(workflow_manager.cv_template_loader_service, CVTemplateLoaderService)
    ...
    def test_workflow_manager_uses_injected_service(self):
        ...
        # Get container and create workflow manager directly
        container = get_container()
        workflow_manager = WorkflowManager(container)
        
        # Mock the cv_template_loader_service.load_from_markdown method
        mock_structured_cv = MagicMock(spec=StructuredCV)
        mock_structured_cv.cv_text = None
        ...
        with tempfile.TemporaryDirectory() as temp_sessions_dir:
            ...
            with patch.object(workflow_manager.cv_template_loader_service, 'load_from_markdown', return_value=mock_structured_cv) as mock_load:
                ...
                mock_load.assert_called_once_with("src/templates/default_cv_template.md")
...
"""
    ...
    def test_multiple_workflow_managers_share_same_service(self):
        ...
        wm1 = WorkflowManager(container)
        wm2 = WorkflowManager(container)
