...
class TestQualityAssuranceAgent:
    ...
    def sample_structured_cv(self):
        ...
        cv.sections = [
            Section(
                ...
                items=[
                    Item(content="Senior Software Developer at XYZ Corp (2019-2024)", status=ItemStatus.GENERATED),
                    Item(content="Led development of scalable web applications using Python and React", status=ItemStatus.GENERATED),
                    Item(content="Mentored team of 3 junior developers", status=ItemStatus.GENERATED)
                ]
            ),
            Section(
                name="Technical Skills",
                items=[
                    Item(content="Programming Languages: Python, JavaScript, TypeScript", status=ItemStatus.GENERATED),
                    Item(content="Frameworks: React, Django, Flask", status=ItemStatus.GENERATED),
                    Item(content="Cloud Platforms: AWS, Docker, Kubernetes", status=ItemStatus.GENERATED)
                ]
            ),
            Section(
                name="Education",
                items=[
                    Item(content="Bachelor of Science in Computer Science, ABC University (2015-2019)", status=ItemStatus.GENERATED)
                ]
            )
        ]
        return cv
    ...
    async def test_run_async_success(self, qa_agent, execution_context, sample_structured_cv, sample_job_data):
        ...
        with patch.object(qa_agent, '_check_content_relevance') as mock_relevance, \
             patch.object(qa_agent, '_check_skill_alignment') as mock_skills, \
             patch.object(qa_agent, '_check_experience_match') as mock_experience, \
             patch.object(qa_agent, '_check_formatting_quality') as mock_formatting:
            
            mock_relevance.return_value = {"score": 0.85, "issues": []}
            mock_skills.return_value = {"score": 0.90, "missing_skills": [], "matched_skills": ["Python", "React"]}
    ...
    async def test_run_async_missing_cv(self, qa_agent, execution_context, sample_job_data):
    ...
    async def test_run_async_missing_job_data(self, qa_agent, execution_context, sample_structured_cv):
    ...
    async def test_run_as_node_execution_error(self, qa_agent, sample_agent_state):
        ...
        with patch.object(qa_agent, 'run_async') as mock_run:
            ...
            assert len(result_state.error_messages) == 1
            assert "Error during quality assurance: QA check failed" in result_state.error_messages[0]
            assert result_state.quality_check_results is None

    def test_check_content_relevance_high_score(self, qa_agent, sample_structured_cv, sample_job_data):
        """Test content relevance check with high relevance."""
        result = qa_agent._check_content_relevance(sample_structured_cv, sample_job_data)
    ...
    def test_check_skill_alignment_perfect_match(self, qa_agent, sample_structured_cv, sample_job_data):
    ...
    def test_check_skill_alignment_missing_skills(self, qa_agent, sample_job_data):
        ...
        cv.sections = [
            Section(
                name="Technical Skills",
                items=[
                    Item(content="Programming Languages: Python", status=ItemStatus.GENERATED)
                ]
            )
        ]
    ...
    def test_check_experience_match(self, qa_agent, sample_structured_cv, sample_job_data):
    ...
    def test_check_formatting_quality_poor_formatting(self, qa_agent):
        ...
        cv.sections = [
            Section(
                ...
                items=[
                    Item(content="worked at company", status=ItemStatus.GENERATED),  # poor capitalization
                    Item(content="did stuff", status=ItemStatus.GENERATED)  # vague content
                ]
            )
        ]
    ...
    def test_generate_recommendations(self, qa_agent):
        ...
        quality_results = {
            "content_relevance": {"score": 0.6, "issues": ["Content not specific enough"]},
            "skill_alignment": {"score": 0.7, "missing_skills": ["Docker", "Kubernetes"]},
    ...
    async def test_run_as_node_logs_execution(self, mock_logger, qa_agent, sample_agent_state):
        """Test that run_as_node logs execution properly."""
        mock_result = AgentResult(
            success=True,
            output_data={"overall_score": 0.8},
        ...
        with patch.object(qa_agent, 'run_async', return_value=mock_result):
            ...
            assert any("Starting quality assurance" in str(call) for call in mock_logger.info.call_args_list)
