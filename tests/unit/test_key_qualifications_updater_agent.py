...
class TestKeyQualificationsUpdaterAgent:
    ...
    @pytest.mark.asyncio
    async def test_successful_update_with_structured_cv_object(self, agent, structured_cv, generated_qualifications):
    ...
    @pytest.mark.asyncio
    async def test_successful_update_with_structured_cv_dict(self, agent, structured_cv_dict, generated_qualifications):
    ...
    async def test_missing_generated_qualifications(self, agent, structured_cv):
        ...
        assert "error_messages" in result
        assert "Missing required input: generated_key_qualifications" in result["error_messages"][0]
    ...
    async def test_empty_generated_qualifications(self, agent, structured_cv):
        ...
        assert "error_messages" in result
        assert "generated_key_qualifications cannot be empty" in result["error_messages"][0]
    ...
    async def test_invalid_generated_qualifications_type(self, agent, structured_cv):
        ...
        assert "error_messages" in result
        assert "generated_key_qualifications must be a list" in result["error_messages"][0]

    @pytest.mark.asyncio
    async def test_missing_key_qualifications_section(self, agent, generated_qualifications):
    ...
    @pytest.mark.asyncio
    async def test_replaces_existing_qualifications(self, agent, structured_cv, generated_qualifications):
        ...
        assert "Old Qualification 1" not in [item.content for item in updated_qual_section.items]

    def test_validate_inputs_with_valid_data(self, agent, structured_cv, generated_qualifications):
