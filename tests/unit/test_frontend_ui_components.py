...
class TestFrontendUIComponents:
    ...
    def test_display_sidebar_with_valid_api_key(
        self,
        mock_divider,
        mock_success,
        mock_text_input,
    ...
    def test_display_input_form_valid_inputs(
        self, mock_button, mock_checkbox, mock_text_area, mock_header, mock_streamlit
    ):
    ...
    def test_display_input_form_no_api_key(
        self, mock_warning, mock_header, mock_streamlit
    ):
    ...
    def test_display_review_and_edit_tab_with_cv(
        self, mock_markdown, mock_expander, sample_agent_state
    ):
        ...
        with patch(
            "src.frontend.ui_components._display_reviewable_item"
        ) as mock_display_item:
    ...
    def test_display_export_tab_with_output(
        self, mock_open, mock_download, mock_success, sample_agent_state
    ):
    ...
    def test_display_export_tab_no_output(self, mock_info):
        ...
        mock_info.assert_called_with(
            "Generate and finalize your CV to enable export options."
        )
    ...
    @patch("streamlit.button")
    def test_display_reviewable_item(
        self,
        mock_button,
        mock_columns,
        mock_code,
    ...
    @patch("streamlit.button")
    @patch("streamlit.info")
    def test_display_input_form_validation_messages(
        self, mock_info, mock_button, mock_checkbox, mock_text_area, mock_streamlit
    ):
        ...
        mock_info.assert_called_with(
            "💡 Please provide a job description to get started."
        )
        ...
        mock_info.assert_called_with(
            "💡 Please provide your CV content or check 'Start from scratch'."
        )
    ...
    def test_display_input_form_button_disabled_when_processing(self, mock_streamlit):
        ...
        with patch("streamlit.session_state", mock_streamlit), patch(
            "streamlit.text_area"
        ) as mock_text_area, patch("streamlit.checkbox") as mock_checkbox, patch(
            "streamlit.button"
        ) as mock_button:
