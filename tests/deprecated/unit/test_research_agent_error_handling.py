...
class TestResearchAgentErrorHandling:
    ...
    def test_parse_llm_response_empty_response(self):
        ...
        with pytest.raises(
            LLMResponseParsingError, match="Empty or None LLM response received"
        ):
    ...
    def test_parse_llm_response_invalid_json(self):
        ...
        with pytest.raises(
            LLMResponseParsingError, match="Failed to parse LLM response as JSON"
        ):
    ...
    def test_parse_llm_response_missing_required_fields(self):
        ...
        with pytest.raises(
            ...
            match="Failed to extract meaningful data from LLM response",
        ):
    ...
    async def test_perform_research_analysis_parsing_error(self):
        ...
        with patch.object(
            self.research_agent,
            "_parse_llm_response",
            side_effect=LLMResponseParsingError("Parsing failed"),
        ):
            result = await self.research_agent._perform_research_analysis(
                self.valid_job_desc
            )
    ...
    async def test_perform_research_analysis_none_result(self):
        ...
        with patch.object(
            self.research_agent, "_parse_llm_response", return_value=None
        ):
            result = await self.research_agent._perform_research_analysis(
                self.valid_job_desc
            )
        ...
        assert result.status == ResearchStatus.FAILED
        assert (
            "Failed to parse LLM response - no findings extracted"
    ...
    async def test_perform_research_analysis_invalid_type(self):
        ...
        with patch.object(
            self.research_agent, "_parse_llm_response", return_value="wrong_type"
        ):
            result = await self.research_agent._perform_research_analysis(
                self.valid_job_desc
            )
    ...
    async def test_execute_parsing_error_handling(self):
        ...
        with patch.object(
            self.research_agent,
            "_perform_research_analysis",
            side_effect=LLMResponseParsingError("Parse error"),
        ):
            result = await self.research_agent._execute(
                job_description_data=self.valid_job_desc
            )
    ...
    async def test_execute_agent_execution_error_handling(self):
        ...
        with patch.object(
            self.research_agent,
            "_perform_research_analysis",
            side_effect=AgentExecutionError("TestAgent", "Execution error"),
        ):
            result = await self.research_agent._execute(
                job_description_data=self.valid_job_desc
            )
    ...
    async def test_execute_unexpected_error_handling(self):
        ...
        assert not result.success
        assert (
            "An unexpected error occurred during research: Unexpected error"
            in result.error_message
        )
    ...
    async def test_execute_successful_research(self):
        ...
        try:
            successful_findings = ResearchFindings(status=ResearchStatus.SUCCESS)
        ...
        with patch.object(
            self.research_agent,
            "_perform_research_analysis",
            return_value=successful_findings,
        ):
            try:
        ...
        assert "Research analysis completed successfully" in result.metadata.get(
            "message", ""
        )
