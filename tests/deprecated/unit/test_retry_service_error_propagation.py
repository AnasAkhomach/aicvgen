...
class TestCB008ErrorPropagationFix:
    ...
    @pytest.fixture
    def retry_service(
        self, mock_retry_handler, mock_api_key_manager, mock_error_recovery
    ):
    ...
    @pytest.mark.asyncio
    async def test_configuration_error_propagated(
        self, retry_service, mock_retry_handler
    ):
    ...
    @pytest.mark.asyncio
    async def test_error_recovery_failure_propagates_original_error(
        ...
        mock_error_recovery.get_fallback_content.side_effect = TypeError(
            "Recovery failed"
        )

        # Act & Assert
        with pytest.raises(
            ValueError, match="Original error that should be propagated"
        ):
    ...
    @pytest.mark.asyncio
    async def test_error_recovery_key_error_propagates_original(
        self, retry_service, mock_error_recovery
    ):
        ...
        mock_error_recovery.get_fallback_content.side_effect = KeyError(
            "Missing key in recovery"
        )

        # Act & Assert
        with pytest.raises(
            RuntimeError, match="Runtime error that should be propagated"
        ):
            await retry_service.handle_error_with_fallback(
                original_error, ContentType.CV_ANALYSIS, start_time=0.0
            )
    ...
    @pytest.mark.asyncio
    async def test_error_recovery_value_error_propagates_original(
        self, retry_service, mock_error_recovery
    ):
        ...
        mock_error_recovery.get_fallback_content.side_effect = ValueError(
            "Invalid value in recovery"
        )

        # Act & Assert
        with pytest.raises(
            ConnectionError, match="Connection error that should be propagated"
        ):
            await retry_service.handle_error_with_fallback(
                original_error, ContentType.CV_ANALYSIS, start_time=0.0
            )
    ...
    @pytest.mark.asyncio
    async def test_successful_fallback_content_returns_response(
        self, retry_service, mock_error_recovery
    ):
