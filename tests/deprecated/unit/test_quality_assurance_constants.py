...
from src.models.data_models import Item, Section
...
class TestQualityAssuranceConstants:
    ...
    def test_executive_summary_word_count_uses_constants(self):
        ...
        short_summary = Section(
            ...
            items=[
                Item(
                    content="Short summary with only ten words here for testing purposes."
                )
            ],
            subsections=[],
        )
    ...
    def test_executive_summary_word_count_passes_with_sufficient_words(self):
        ...
        long_content = " ".join(
            ["word"] * (AgentConstants.MIN_WORD_COUNT_EXECUTIVE_SUMMARY + 10)
        )
        long_summary = Section(
            name="Executive Summary", items=[Item(content=long_content)], subsections=[]
        )
