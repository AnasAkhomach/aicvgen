...
class MockAgent(AgentBase):
    ...
    def __init__(self, name: str = "MockAgent"):
        super().__init__(
            name=name, description="Mock agent for testing", session_id="test-session"
        )
    ...
    async def _execute(self, **kwargs: Any) -> AgentResult:
        ...
        return AgentResult(success=True, output_data=Mock(), agent_name=self.name)
...
class TestAgentBaseInputMapping:
    ...
    async def test_run_as_node_with_valid_inputs(self, mock_agent, mock_state):
        ...
        try:
            ...
            assert (
                "current_item_id" not in mock_agent.received_kwargs
            )  # Not in CVAnalyzerAgentInput
    ...
    async def test_run_as_node_input_extraction_failure(self, mock_agent, mock_state):
        ...
        assert any(
            "Input extraction failed" in msg for msg in call_args["error_messages"]
        )
...
class TestArchitecturalImprovements:
    ...
    def test_agent_input_models_registry_completeness(self):
        ...
        for agent_name in expected_agents:
            assert (
                agent_name in AGENT_INPUT_MODELS
            ), f"Missing input model for {agent_name}"
