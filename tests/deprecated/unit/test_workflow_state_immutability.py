...
from src.models.agent_output_models import CVAnalysisResult, ResearchFindings
...
class TestCB009StateImmutabilityFix:
    ...
    @pytest.mark.asyncio
    async def test_jd_parser_node_error_immutability(
        ...
        workflow_graph.job_description_parser_agent.run_as_node.side_effect = (
        ...
        assert (
            "JobDescriptionParserAgent failed: Test error"
            in result_state.error_messages
        )
        assert base_agent_state is not result_state  # Different objects

    @pytest.mark.asyncio
    async def test_cv_parser_node_error_immutability(
        self, workflow_graph, base_agent_state
    ):
        ...
        workflow_graph.user_cv_parser_agent.run_as_node.side_effect = RuntimeError(
            "Test error"
        )
    ...
    @pytest.mark.asyncio
    async def test_supervisor_node_metadata_immutability(
        self, workflow_graph, base_agent_state
    ):
        ...
        # Assert
        assert (
            base_agent_state.node_execution_metadata == original_metadata
        )  # Original unchanged
        ...
        assert (
            result_state.node_execution_metadata["existing_key"] == "existing_value"
        )  # Preserved
        assert base_agent_state is not result_state  # Different objects

    @pytest.mark.asyncio
    async def test_mark_subgraph_completion_node_metadata_immutability(
        self, workflow_graph, base_agent_state
    ):
        ...
        # Act
        result_state = await workflow_graph.mark_subgraph_completion_node(
            base_agent_state
        )
    ...
    @pytest.mark.asyncio
    async def test_entry_router_node_metadata_immutability(
        self, workflow_graph, base_agent_state
    ):
    ...
    @pytest.mark.asyncio
    async def test_error_handler_node_error_immutability(
        self, workflow_graph, base_agent_state
    ):
        ...
        with patch(
            "src.orchestration.cv_workflow_graph.ErrorRecoveryService"
        ) as mock_service:
            mock_service.return_value.handle_error.side_effect = Exception(
                "Recovery failed"
            )
            ...
            # Assert
            assert (
                base_agent_state.error_messages == original_errors
            )  # Original unchanged
            assert len(result_state.error_messages) == len(original_errors) + 1
            assert (
                "Error handler failed: Recovery failed" in result_state.error_messages
            )
            assert base_agent_state is not result_state  # Different objects

    @pytest.mark.asyncio
    async def test_formatter_node_error_messages_immutability(
        self, workflow_graph, base_agent_state
    ):
    ...
    @pytest.mark.asyncio
    async def test_state_objects_are_never_same_reference(
        self, workflow_graph, base_agent_state
    ):
        ...
        for node_method in nodes_to_test:
            ...
            assert (
                result_state is not base_agent_state
            ), f"{node_method.__name__} returned same object reference"
