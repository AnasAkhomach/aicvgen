...
async def test_node_compliance():
    ...
    with patch(
        ...
        node_functions = [name for name in node_methods]

        print(f"Found {len(node_functions)} node functions to test")

        # Create test state
        test_state = AgentState(cv_text="Test CV content", workflow_status="PROCESSING")

        # Create error state for error testing
        error_state = AgentState(cv_text="Test CV content", workflow_status="ERROR")
        ...
        if isinstance(error_result, dict):
            ...
            print(
                f"❌ error_handler_node: Unexpected error handling result: {error_result}"
            )
            test_4_passed = False
        ...
        print(
            f"2. @validate_node_output decorators: {'✅ PASS' if test_2_passed else '❌ FAIL'}"
        )
