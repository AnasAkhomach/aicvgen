...
from src.models.cv_models import Item, ItemStatus, Section, StructuredCV
...
class TestProjectsUpdaterAgent:
        ...
        return StructuredCV(
            sections=[
                Section(
                    name="Projects",
                    items=[
                        Item(content="Old projects content", status=ItemStatus.INITIAL)
                    ],
                ),
    ...
    def agent_input(self, mock_structured_cv, generated_projects_data):
        ...
        return ProjectsUpdaterAgentInput(
            structured_cv=mock_structured_cv, generated_projects=generated_projects_data
        )
    ...
    def updater_agent(self):
        ...
        return ProjectsUpdaterAgent(
            session_id="test_session", name="test_projects_updater"
        )
    ...
    def test_input_validation_missing_structured_cv(
        self, updater_agent, generated_projects_data
    ):
        """Test input validation fails when structured_cv is missing."""
        invalid_input = ProjectsUpdaterAgentInput(
            structured_cv=None, generated_projects=generated_projects_data
        )
    ...
    def test_input_validation_missing_generated_data(
        ...
        invalid_input = ProjectsUpdaterAgentInput(
            structured_cv=mock_structured_cv, generated_projects=None
        )
    ...
    def test_find_projects_section_not_exists(self, updater_agent):
        ...
        cv_without_projects = StructuredCV(
            sections=[Section(title="Education", items=[])]
        )
    ...
    def test_update_section_with_generated_data(
        self, updater_agent, generated_projects_data
    ):
        """Test updating section with generated projects data."""
        section = Section(title="Projects", items=[])

        updater_agent._update_section_with_generated_data(
            section, generated_projects_data
        )
        ...
        assert section.items[0].metadata == {
        ...
        assert section.items[1].metadata == {
    ...
    async def test_execute_with_empty_generated_data(
        ...
        agent_input = ProjectsUpdaterAgentInput(
            structured_cv=mock_structured_cv, generated_projects=""
        )
