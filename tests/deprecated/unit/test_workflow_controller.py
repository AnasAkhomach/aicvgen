...
class TestWorkflowController(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_agent_state = AgentState(
            structured_cv=structured_cv, cv_text="Sample CV text"
        )
    ...
    def test_start_generation(self, mock_uuid, mock_session_state):
        ...
        with patch("asyncio.run_coroutine_threadsafe") as mock_run_coroutine:
            ...
            self.assertEqual(
                call_args[0][1], self.mock_loop
            )  # Second arg should be the loop
    ...
    def test_submit_user_feedback_not_ready(self):
        ...
        with patch("streamlit.error") as mock_error:
            result = controller.submit_user_feedback(
                action="accept", item_id="test-item", workflow_session_id="test-session"
            )
    ...
    def test_shutdown_exception_handling(self):
        ...
        self.mock_loop.call_soon_threadsafe.side_effect = Exception(
    ...
    def test_del_method_exception_handling(self):
        ...
        with patch.object(
            controller, "shutdown", side_effect=Exception("Shutdown failed")
        ):
            with patch("builtins.print") as mock_print:
                ...
                self.assertIn(
                    "Error in WorkflowController.__del__", mock_print.call_args[0][0]
                )
