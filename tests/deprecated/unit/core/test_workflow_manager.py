...
class TestAsyncWorkflowManager:
    ...
    def mock_workflow_wrapper(self, mock_container):
        ...
        with patch(
            "src.core.workflow_manager.create_cv_workflow_graph_with_di"
        ) as mock_create:
    ...
    @patch("builtins.open", new_callable=mock_open)
    @patch("src.core.workflow_manager.Path")
    def test_create_duplicate_workflow_raises_error(
        self, mock_path, mock_file, mock_container
    ):
        ...
        with pytest.raises(
            ValueError, match=f"Workflow with session_id {session_id} already exists"
        ):
            manager.create_new_workflow(cv_text, jd_text, session_id)

    @pytest.mark.asyncio
    async def test_trigger_workflow_step_success(
        self, workflow_manager, mock_workflow_wrapper
    ):
        ...
        with patch.object(workflow_manager, "get_workflow_status") as mock_get_status:
            ...
            with patch.object(
                workflow_manager, "_get_workflow_graph"
            ) as mock_get_graph:
    ...
    async def test_trigger_workflow_step_nonexistent_workflow(self, workflow_manager):
        ...
        with patch.object(workflow_manager, "get_workflow_status") as mock_get_status:
            ...
            with pytest.raises(
                ValueError, match="No active workflow found for session_id: nonexistent"
            ):
                await workflow_manager.trigger_workflow_step(
                    "nonexistent", global_state
                )

    @pytest.mark.asyncio
    async def test_trigger_workflow_step_with_exception(
        self, workflow_manager, mock_workflow_wrapper
    ):
        ...
        with patch.object(workflow_manager, "get_workflow_status") as mock_get_status:
            ...
            with patch.object(
                ...
                with patch("builtins.open", mock_open()) as mock_file:
                    ...
                    with pytest.raises(
                        RuntimeError,
                        match="Workflow execution failed: Graph execution failed",
                    ):
                        await workflow_manager.trigger_workflow_step(
                            session_id, global_state
                        )
    ...
    @patch(
        ...
        read_data='{"session_id": "test-session", "cv_text": "Test CV", "structured_cv": {}, "error_messages": [], "trace_id": "test-trace", "node_execution_metadata": {}, "workflow_status": "PROCESSING", "ui_display_data": {}, "automated_mode": false}',
    )
    @patch("src.core.workflow_manager.Path")
    ...
    @patch("src.core.workflow_manager.Path")
    def test_get_workflow_status_nonexistent(
        self, mock_path, mock_file, workflow_manager
    ):
    ...
    @patch("src.core.workflow_manager.Path")
    def test_send_feedback_existing_workflow(
        self, mock_path, mock_file, workflow_manager
    ):
    ...
    def test_determine_next_stage_internal(self, workflow_manager):
        ...
        next_stage = workflow_manager._determine_next_stage(
    ...
    async def test_workflow_wrapper_mocking(self, mock_workflow_wrapper):
        ...
        # Test ainvoke method
        test_state = {"trace_id": "test-trace", "cv_text": "Test CV content"}
