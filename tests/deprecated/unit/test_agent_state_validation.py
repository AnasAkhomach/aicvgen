...
class TestAgentStateValidation:
    ...
    def valid_agent_state(self):
        ...
        return AgentState(structured_cv=structured_cv, cv_text="Test CV text content")

    def test_set_user_feedback_valid(self, valid_agent_state):
        """Test setting valid user feedback."""
        state = valid_agent_state
        feedback = UserFeedback(
            action=UserAction.ACCEPT, item_id="test_item", feedback_text="Good work"
        )
    ...
    def test_set_user_feedback_invalid_type(self, valid_agent_state):
        ...
        with pytest.raises(
            ValueError, match="feedback must be a UserFeedback instance"
        ):
    ...
    def test_set_research_findings_invalid_type(self, valid_agent_state):
        ...
        with pytest.raises(
            ValueError, match="findings must be a ResearchFindings instance"
        ):
    ...
    def test_set_quality_check_results_invalid_type(self, valid_agent_state):
        ...
        with pytest.raises(
            ValueError, match="results must be a QualityAssuranceAgentOutput instance"
        ):
    ...
    def test_set_cv_analysis_results_invalid_type(self, valid_agent_state):
        ...
        with pytest.raises(
            ValueError, match="results must be a CVAnalysisResult instance"
        ):
