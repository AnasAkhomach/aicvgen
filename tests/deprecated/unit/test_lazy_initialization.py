...
class TestLazyInitialization:
    ...
    def test_create_llm_api_key_manager_lazy_success(self):
        ...
        with patch(
            "src.core.factories.service_factory.LLMApiKeyManager"
    ...
    def test_create_llm_api_key_manager_lazy_invalid_settings(self):
        ...
        with pytest.raises(ServiceInitializationError) as exc_info:
            ServiceFactory.create_llm_api_key_manager_lazy(
                settings=None, llm_client=mock_llm_client
            )

        assert "Settings dependency is None or invalid" in str(exc_info.value)
        assert (
            exc_info.value.context.additional_data["service_name"]
            == "llm_api_key_manager"
        )
    ...
    def test_create_llm_api_key_manager_lazy_invalid_client(self):
        ...
        with pytest.raises(ServiceInitializationError) as exc_info:
            ServiceFactory.create_llm_api_key_manager_lazy(
                settings=mock_settings, llm_client=None
            )
    ...
    def test_create_llm_retry_service_lazy_success(self):
        ...
        with patch(
            "src.core.factories.service_factory.LLMRetryService"
    ...
    def test_create_enhanced_llm_service_lazy_success(self):
        ...
        with patch(
            "src.core.factories.service_factory.EnhancedLLMService"
        ) as mock_service_class:
            mock_service = Mock()
            mock_service_class.return_value = mock_service
    ...
    def test_lazy_initialization_exception_handling(self):
        ...
        with patch(
            ...
            with pytest.raises(ServiceInitializationError) as exc_info:
                ServiceFactory.create_llm_api_key_manager_lazy(
                    settings=mock_settings, llm_client=mock_llm_client
                )

            assert "Initialization failed: Service creation failed" in str(
                exc_info.value
            )
            assert (
                exc_info.value.context.additional_data["service_name"]
