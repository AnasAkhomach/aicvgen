...
class TestInteractiveUILoop:
    ...
    def setup_method(self):
        ...
        self.mock_container.workflow_manager = Mock(
            return_value=self.mock_workflow_manager
        )
    ...
    @patch("asyncio.run")
    def test_on_start_generation_success(
        self, mock_asyncio_run, mock_st_error, mock_session_state, mock_get_container
    ):
        ...
        self.mock_workflow_manager.get_workflow_status.return_value = (
        ...
        self.mock_workflow_manager.get_workflow_status.assert_called_once_with(
            "test_session_id"
        )
    ...
    @patch("streamlit.session_state")
    @patch("streamlit.error")
    def test_on_start_generation_missing_inputs(
        self, mock_st_error, mock_session_state, mock_get_container
    ):
        ...
        mock_st_error.assert_called_once_with(
            "Please provide both CV text and job description."
        )
    ...
    @patch("asyncio.run")
    def test_on_approve_success(
        self, mock_asyncio_run, mock_st_success, mock_session_state, mock_get_container
    ):
        ...
        mock_st_success.assert_called_once_with(
            "✅ Content approved and workflow resumed."
        )
    ...
    @patch("asyncio.run")
    def test_on_regenerate_success(
        self, mock_asyncio_run, mock_st_info, mock_session_state, mock_get_container
    ):
        ...
        mock_st_info.assert_called_once_with(
            "🔄 Content regeneration requested and workflow resumed."
        )
    ...
    def test_render_awaiting_feedback_ui(
        self, mock_button, mock_columns, mock_text_area, mock_write, mock_subheader
    ):
    ...
    def test_render_completed_ui_with_file(
        self, mock_file_open, mock_download_button, mock_success
    ):
    ...
    def test_render_completed_ui_no_file(self, mock_info, mock_success):
        ...
        mock_info.assert_called_once_with(
            "CV generation completed but download file is not available."
        )
    ...
    @patch(
        "src.error_handling.boundaries.safe_streamlit_component",
    ...
    def test_main_no_session_id(
        ...
        mock_markdown.assert_called_once_with(
            "Generate tailored CVs using AI-powered analysis"
        )
    ...
    @patch("streamlit.title")
    @patch("streamlit.markdown")
    def test_main_with_awaiting_feedback_status(
        self,
        mock_markdown,
        mock_title,
        mock_session_state,
        mock_display_sidebar,
        mock_render_awaiting_feedback,
        mock_state_manager_class,
        mock_get_container,
    ):
    ...
    def test_main_with_processing_status(
        self,
        mock_rerun,
        mock_sleep,
        mock_spinner,
