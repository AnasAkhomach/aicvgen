...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('test_pipeline.log', mode='w')  # Use mode='w' to overwrite previous log
    ]
)
...
class VerbosePromptLoader(PromptLoader):
    ...
    def load_prompt(self, prompt_name):
        ...
        print(f"Loading prompt: {prompt_name}")  # Print directly for immediate visibility
        prompt_text = super().load_prompt(prompt_name)
        logger.info(f"Loaded prompt: {prompt_name}")
        logger.info(f"Prompt content (first 200 chars): {prompt_text[:200].replace('{', '{{').replace('}', '}}')}")
        return prompt_text
    ...
    def get_executive_summary_prompt(self, job_data, cv_data, research_results):
        ...
        result = super().get_executive_summary_prompt(job_data, cv_data, research_results)
    ...
    def get_side_project_prompt(self, job_data, project_data, research_results):
        ...
        result = super().get_side_project_prompt(job_data, project_data, research_results)
...
class VerboseContentWriterAgent(ContentWriterAgent):
    ...
    def run(self, input_data):
        ...
        if "research_results" in input_data and input_data["research_results"]:
            print(f"Research result keys: {list(input_data['research_results'].keys())}")
            if "key_matches" in input_data["research_results"]:
                print(f"Key matches types: {list(input_data['research_results']['key_matches'].keys())}")
...
def main():
    ...
    cv_analyzer_agent = CVAnalyzerAgent(
        ...
        description="Agent for analyzing user CVs.", 
        llm=llm
    )
        ...
        if "structured_cv" in result:
            ...
            if hasattr(structured_cv, "metadata"):
                ...
                if "linkedin" in structured_cv.metadata:
                    contact_parts.append(f"🔗 [LinkedIn]({structured_cv.metadata['linkedin']})")
                if "github" in structured_cv.metadata:
                    contact_parts.append(f"💻 [GitHub]({structured_cv.metadata['github']})")
            ...
            with open(cv_path, "w", encoding="utf-8") as f:
                f.write(cv_text)
            logger.info(f"Generated simple CV from structured data and saved to {cv_path}")
