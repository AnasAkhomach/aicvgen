...
def test_dependency_injection():
    ...
    try:
        ...
        workflow_graph = create_cv_workflow_graph_with_di(container)
        
        print("✓ Successfully created workflow graph with dependency injection")
        
        # Verify that the graph has the expected methods (updated for current architecture)
        assert hasattr(workflow_graph, 'invoke'), "Graph missing 'invoke' method"
        assert hasattr(workflow_graph, 'ainvoke'), "Graph missing 'ainvoke' method"
        
        print("✓ Graph has all expected methods for current architecture")
