...
from core.agent_lifecycle_manager import get_agent_lifecycle_manager, reset_agent_lifecycle_manager
...
class OptimizationDemo:
    ...
    async def _demo_startup_optimization(self) -> None:
        ...
        print(f"   Fastest startup: {report['performance_summary']['fastest_startup']:.3f}s")
        print(f"   Slowest startup: {report['performance_summary']['slowest_startup']:.3f}s")
        print(f"   Average startup: {report['performance_summary']['average_startup_time']:.3f}s")
    ...
    async def _demo_dependency_injection(self) -> None:
        ...
        container.register_session("user_session", UserSessionService, factory=UserSessionService)
    ...
    async def _demo_performance_analysis(self) -> None:
        ...
        if "system_performance" in summary:
            ...
            if "memory_usage_mb" in sys_perf:
                print(f"   Memory Usage: {sys_perf['memory_usage_mb']['current']:.1f}MB")
        
        if "operation_performance" in summary:
            print("\n   Operation Performance:")
            for op, stats in summary["operation_performance"].items():
                print(f"     {op}: {stats['average_time_ms']:.1f}ms avg ({stats['total_executions']} runs)")
        ...
        self.monitor.set_baseline("operation_content_generation_time", 2000)  # 2 seconds
        ...
        for operation in ["cv_parsing", "content_generation"]:
            ...
            if comparison:
                ...
                if improvement > 0:
                    print(f"   ✅ {operation}: {improvement:.1f}% improvement vs baseline")
                else:
                    print(f"   ⚠️  {operation}: {abs(improvement):.1f}% slower than baseline")
    ...
    async def _generate_final_report(self) -> None:
        ...
        try:
            export_path = Path(__file__).parent.parent / "logs" / f"optimization_demo_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
