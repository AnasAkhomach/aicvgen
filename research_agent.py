...
from llm import LLM
from state_manager import JobDescriptionData, AgentIO, StructuredCV, Section, Subsection, Item
...
class ResearchAgent(AgentBase):
    ...
    def run(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(f"Running ResearchAgent for job with {len(skills)} skills, {len(responsibilities)} responsibilities")
        ...
        if self.vector_db:
            ...
            research_results["relevance_scores"] = self._calculate_section_relevance(
                structured_cv,
                skills + responsibilities + industry_terms
            )
        ...
        logger.info(f"ResearchAgent completed analysis with {len(research_results['key_matches'])} match categories")
