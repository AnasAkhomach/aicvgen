...
async def initialize_supervisor_node(state: GlobalState) -> GlobalState:
    ...
    try:
        ...
        supervisor_state = _initialize_supervisor_state(
            state["structured_cv"], WORKFLOW_SEQUENCE
        )
...
async def supervisor_node(state: GlobalState) -> GlobalState:
    ...
    try:
        ...
        # Handle None current_section_index (no sections with items)
        elif current_section_index is None:
            logger.info("No sections with items found, workflow complete (MVP)")
            next_node = "END"  # MVP: Direct completion instead of formatter
        ...
        # Check if all sections are completed
            ...
            logger.info("All content sections completed, workflow complete (MVP)")
            next_node = "END"  # MVP: Direct completion instead of formatter
        else:
            ...
            if state.get("subgraph_completed", False):
                ...
                while (
                    current_section_index < len(workflow_sections)
                    and not workflow_sections[current_section_index]
                ):
                    current_section_index += 1
            ...
            if current_section_index < len(workflow_sections):
                ...
                next_node = "END"  # MVP: Direct completion instead of formatter
                logger.info("All sections completed, workflow complete (MVP)")
        ...
        node_execution_metadata.update(
            {"next_node": next_node, "last_executed_node": "SUPERVISOR"}
        )
...
async def entry_router_node(state: GlobalState) -> GlobalState:
    ...
    try:
        ...
        if has_job_data and has_research and has_analysis:
            ...
            # Start from CV parsing
            next_node = "USER_CV_PARSER"
            logger.info("No existing data, starting from CV parser")
...
def _initialize_supervisor_state(
    structured_cv, workflow_sequence: List[str]
) -> Dict[str, Any]:
    ...
    if (
        not structured_cv
        or not hasattr(structured_cv, "sections")
        or not structured_cv.sections
    ):
        return {
            "workflow_sections": [],
    ...
    return {
        ...
        "supervisor_metadata": {
            "supervisor_initialized": True,
            "available_sections": [
                section.name for section in structured_cv.sections if section.items
            ],
        },
    }
