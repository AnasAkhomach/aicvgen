...
class Orchestrator:
    ...
    @classmethod
    def create_default(cls, user_api_key: str = None):
        """
        Create an Orchestrator with default agents, optionally using a user-provided API key.
        
        Args:
            user_api_key: Optional user-provided Gemini API key
            
        Returns:
            Orchestrator instance with all agents initialized
        """
        # Create LLM service with user API key if provided
        from src.services.llm import EnhancedLLMService
        llm = EnhancedLLMService(user_api_key=user_api_key)
        
        # Create all required agents
        parser_agent = ParserAgent(
            name="Parser Agent",
            description="Agent responsible for parsing job descriptions and CVs",
            llm=llm
        )
        template_renderer = TemplateRenderer(
            ...
            model=llm,
            input_schema=AgentIO(description="CV content data"),
            output_schema=AgentIO(description="Rendered CV in markdown format")
        )
        # Create tools agent first as it's needed by other agents
        tools_agent = ToolsAgent(
            name="Tools Agent",
            description="Agent responsible for providing tool access for content processing"
        )
        
        # Create vector database instance
        from src.services.vector_db import VectorDB
        from src.core.state_manager import VectorStoreConfig
        vector_config = VectorStoreConfig(dimension=768, index_type="IndexFlatL2")
        vector_db = VectorDB(config=vector_config)
        
        vector_store_agent = VectorStoreAgent(
            name="Vector Store Agent",
            description="Agent for interacting with vector store",
            model=llm,
            input_schema=AgentIO(description="Vector store operations"),
            output_schema=AgentIO(description="Vector store results"),
            vector_db=vector_db
        )
        
        content_writer_agent = ContentWriterAgent(
            name="Content Writer Agent",
            description="Agent responsible for generating tailored CV content",
            llm=llm,
        ...
        research_agent = ResearchAgent(
            name="Research Agent",
            description="Agent responsible for conducting research related to job descriptions",
            llm=llm,
            vector_db=vector_db
        )
        ...
        cv_analyzer_agent = CVAnalyzerAgent(
            ...
            description="Agent responsible for analyzing CV content",
            llm=llm
        )
        
        formatter_agent = FormatterAgent(
            name="Formatter Agent",
            description="Agent responsible for formatting CV content"
        )
