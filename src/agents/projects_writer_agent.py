...
class ProjectsWriterAgentInput(BaseModel):
    ...
    template_content: str = Field(
        description="Formatted template content for the prompt"
    )
    format_instructions: str = Field(
        description="Output format instructions from parser"
    )
...
class ProjectsWriterAgent(AgentBase):
    ...
    async def _execute(self, **kwargs: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            generated_data: ProjectLLMOutput = await self.chain.ainvoke(
