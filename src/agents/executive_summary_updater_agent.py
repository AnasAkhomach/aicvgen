...
class ExecutiveSummaryUpdaterAgent(AgentBase):
    ...
    def _validate_inputs(self, kwargs: dict[str, Any]) -> None:
        ...
        if not isinstance(generated_summary, (dict, str)):
            raise AgentExecutionError(
                self.name, "generated_executive_summary must be a dict or string"
            )

        if not generated_summary:
            raise AgentExecutionError(
                self.name, "generated_executive_summary cannot be empty"
            )
    ...
    async def _execute(self, **kwargs: Any) -> dict[str, Any]:
        ...
        try:
            ...
            if isinstance(generated_summary, dict):
                ...
                content = generated_summary.get(
                    "summary", generated_summary.get("content", str(generated_summary))
                )
            else:
            ...
            return {
                "error_messages": [
                    f"Unexpected error during Executive Summary update: {str(e)}"
                ]
            }
