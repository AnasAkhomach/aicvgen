...
class ProfessionalExperienceUpdaterAgent(AgentBase):
    ...
    def __init__(self, session_id: str, name: str = "ProfessionalExperienceUpdaterAgent"):
    ...
    def _validate_inputs(self, kwargs: dict[str, Any]) -> None:
        ...
        if not isinstance(generated_exp, (dict, str)):
            raise AgentExecutionError(self.name, "generated_professional_experience must be a dict or string")
            
        if not generated_exp:
            raise AgentExecutionError(self.name, "generated_professional_experience cannot be empty")
    ...
    async def _execute(self, **kwargs: Any) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Updated Professional Experience section with generated content")
            ...
            return {"error_messages": [f"Error updating Professional Experience: {str(e)}"]}
        except Exception as e:
            logger.error(f"Unexpected error in {self.name}: {str(e)}", exc_info=True)
            return {"error_messages": [f"Unexpected error during Professional Experience update: {str(e)}"]}