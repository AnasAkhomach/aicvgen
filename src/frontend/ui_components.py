...
def display_sidebar(state: AgentState):
    ...
    with st.sidebar:
        ...
        if user_api_key and user_api_key != st.session_state.get('user_gemini_api_key', ''):
        ...
        if (
            ...
            session_usage_pct = (
                st.session_state.get('session_tokens_used', 0) / st.session_state.session_token_limit
            ) * 100
...
def display_input_form(state: Optional[AgentState]):
    ...
    if not st.session_state.get('user_gemini_api_key'):
        st.warning("⚠️ Please enter your Gemini API key in the sidebar before proceeding.")
        return
    ...
    # Generate button
    can_generate = bool(job_description.strip()) and (bool(cv_content.strip()) or start_from_scratch)
