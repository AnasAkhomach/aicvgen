            ...
            for other_key, other_accesses in self._access_patterns.items():
                ...
                for other_time in other_accesses:
                    if access_time < other_time <= access_time + 3600:  # Within 1 hour
                        ...
                        score = 1.0 / (1.0 + time_diff / 3600)  # Closer in time = higher score
            ...
            if re.match(pattern, key):
                for callback in callbacks:
                    try:
                        ...
                        logger.warning("Error in invalidation callback", pattern=pattern, error=str(e))
...
class IntelligentCacheManager:
    ...
    def __init__(self, 
        ...
        self._level_configs = {
            ...
            CacheLevel.L4_DISTRIBUTED: {"max_entries": 100000, "ttl_hours": 168}  # 1 week
        }
        ...
        logger.info("Intelligent cache manager initialized", max_memory_mb=max_memory_mb)
    ...
    def set(self, key: str, value: Any, 
           ...
           priority: int = 1,
           pattern: CachePattern = CachePattern.MIXED) -> bool:
    ...
    def _determine_cache_level(self, size_bytes: int, priority: int, pattern: CachePattern) -> CacheLevel:
        ...
        if size_bytes < 102400 and pattern in [CachePattern.READ_HEAVY, CachePattern.TEMPORAL]:
    ...
    def _can_fit_entry(self, level: CacheLevel, entry: CacheEntry) -> bool:
        ...
        if level == CacheLevel.L1_MEMORY:
            ...
            if current_size + entry.size_bytes > self.max_memory_bytes * 0.3:  # 30% for L1
                return False
    ...
    def _promote_entry(self, key: str, entry: CacheEntry, current_level: CacheLevel):
        ...
        level_order = [CacheLevel.L1_MEMORY, CacheLevel.L2_COMPRESSED, 
                      CacheLevel.L3_PERSISTENT, CacheLevel.L4_DISTRIBUTED]
        ...
        with self._lock:
            ...
            return {
                ...
                "memory_usage_percent": round(
                    (total_stats.size_bytes / self.max_memory_bytes) * 100, 2
...
def intelligent_cache(ttl_hours: int = 1, 
                     tags: Optional[Set[str]] = None,
                     priority: int = 1,
                     pattern: CachePattern = CachePattern.MIXED):
