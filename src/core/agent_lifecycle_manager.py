...
from .dependency_injection import (
    DependencyContainer, get_container, LifecycleScope, DependencyMetadata
)
...
class AgentLifecycleManager:
    ...
    def _register_core_dependencies(self):
        ...
        self.container.register_singleton(
            "error_recovery", type(get_error_recovery_service()), 
            factory=get_error_recovery_service
        )
        
        self.container.register_singleton(
            "progress_tracker", type(get_progress_tracker()), 
            factory=get_progress_tracker
        )
        
        self.container.register_singleton(
            "session_manager", type(get_session_manager()), 
    ...
    def register_agent_type(
        ...
        with self._lock:
            ...
            scope = LifecycleScope.SESSION if config.strategy == AgentPoolStrategy.FIXED else LifecycleScope.TRANSIENT
    ...
    def get_agent(
        ...
        with self._lock:
            ...
            logger.warning(f"No available agents for type {agent_type}, pool at capacity")
            return None
    
    def _get_available_agent(
        self, 
        pool: List[ManagedAgent], 
        content_type: Optional[ContentType] = None
    ) -> Optional[ManagedAgent]:
    ...
    def _create_agent(
        ...
        try:
            ...
            logger.info(f"Agent created: {config.agent_type}", 
                       creation_time=creation_time, session_id=session_id)
            ...
            self._error_handler.handle_error(
                error_msg, ErrorCategory.AGENT_LIFECYCLE, ErrorSeverity.HIGH,
    ...
    def dispose_session_agents(self, session_id: str):
        ...
        with self._lock:
            ...
            logger.info(f"Session agents disposed", 
                       session_id=session_id, count=disposed_count)
    ...
    def warmup_pools(self):
        ...
        with self._lock:
            ...
            logger.info(f"Agent pools warmed up", 
                       agents_created=total_created, warmup_time=warmup_time)
    ...
    def _cleanup_idle_agents(self):
        ...
        with self._lock:
            ...
            for agent_type, pool in self._pools.items():
                ...
                for agent in pool:
                    if (len(pool) > config.min_instances and 
                        agent.is_idle_timeout_exceeded()):
                        agents_to_dispose.append(agent)
    ...
    def _update_performance_metrics(self):
        ...
        with self._lock:
            ...
            busy_agents = sum(
                ...
                for agent in pool if agent.state == AgentState.BUSY
            )
            
            if total_agents > 0:
                self._global_metrics["average_pool_utilization"] = busy_agents / total_agents
        ...
        with self._lock:
            ...
            for agent_type, pool in self._pools.items():
                ...
                pool_stats[agent_type] = {
                    ...
                    "max_instances": config.max_instances,
                    "strategy": config.strategy.value,
                    "busy_agents": sum(1 for agent in pool if agent.state == AgentState.BUSY),
                    "idle_agents": sum(1 for agent in pool if agent.state == AgentState.IDLE),
                    "ready_agents": sum(1 for agent in pool if agent.state == AgentState.READY)
                }
