...
def update_processing_state(processing: bool, step: str = None, progress: int = None, message: str = None) -> None:
    ...
    logger.info(f"Processing state updated: {processing}, step: {step}, progress: {progress}%")
...
def update_token_usage(session_tokens: int = 0, daily_tokens: int = 0) -> None:
    ...
    logger.info(f"Token usage updated: +{session_tokens} session, +{daily_tokens} daily")
...
def clear_regeneration_flag(item_type: str, item_id: str) -> None:
    ...
    if "regeneration_flags" in st.session_state and key in st.session_state.regeneration_flags:
