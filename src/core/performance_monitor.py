...
class PerformanceMonitor:
        ...
        while self.monitoring_active:
            try:
                ...
                self.error_handler.handle_error(
                    ...
                    ErrorCategory.SYSTEM_MONITORING,
    ...
    async def _collect_agent_stats(self) -> None:
        ...
        try:
            ...
            for agent_type, agent_stats in stats.get("agent_pools", {}).items():
                ...
                pool_utilization = (active_instances / max_instances) * 100 if max_instances > 0 else 0
                ...
                # Record as metrics
                self.record_metric(f"agent_{agent_type}_response_time", avg_response_time, "ms")
                self.record_metric(f"agent_{agent_type}_pool_utilization", pool_utilization, "%")
                self.record_metric(f"agent_{agent_type}_active_instances", active_instances, "count")
    ...
    async def _check_thresholds(self) -> None:
        ...
        try:
            ...
            if latest_snapshot.memory_percent > self.thresholds["memory_percent"]:
                alerts.append(f"High memory usage: {latest_snapshot.memory_percent:.1f}%")
            ...
            for agent_type, stats in self.agent_stats.items():
                if stats.average_response_time > self.thresholds["response_time_ms"]:
                    alerts.append(f"Slow response time for {agent_type}: {stats.average_response_time:.1f}ms")
                
                error_rate = (stats.failed_requests / stats.total_requests * 100) if stats.total_requests > 0 else 0
                if error_rate > self.thresholds["error_rate_percent"]:
                    alerts.append(f"High error rate for {agent_type}: {error_rate:.1f}%")
                
                if stats.pool_utilization > self.thresholds["pool_utilization_percent"]:
                    alerts.append(f"High pool utilization for {agent_type}: {stats.pool_utilization:.1f}%")
    ...
    def record_metric(self, name: str, value: float, unit: str, context: Optional[Dict[str, Any]] = None) -> None:
    ...
    def record_error(self, error_type: str) -> None:
        ...
        self.record_metric(f"error_{error_type}_count", self.error_counts[error_type], "count")
    ...
    def _calculate_system_summary(self, snapshots: List[SystemSnapshot]) -> Dict[str, Any]:
    ...
    def _generate_performance_recommendations(self) -> List[str]:
        ...
        if self.system_snapshots:
            ...
            if latest.cpu_percent > 70:
                recommendations.append("High CPU usage detected. Consider optimizing agent algorithms or reducing concurrent operations.")
            
            if latest.memory_percent > 75:
                recommendations.append("High memory usage detected. Consider implementing more aggressive cleanup or reducing agent pool sizes.")
        
        # Check agent performance
        for agent_type, stats in self.agent_stats.items():
            if stats.average_response_time > 3000:  # 3 seconds
                recommendations.append(f"Agent {agent_type} has slow response times. Consider optimization or scaling.")
            
            if stats.pool_utilization > 80:
                recommendations.append(f"Agent {agent_type} pool is highly utilized. Consider increasing pool size.")
            
            error_rate = (stats.failed_requests / stats.total_requests * 100) if stats.total_requests > 0 else 0
            if error_rate > 3:
                recommendations.append(f"Agent {agent_type} has high error rate ({error_rate:.1f}%). Investigate error causes.")
        ...
        for operation, times in self.operation_times.items():
            if times and len(times) > 10:
                ...
                if avg_time > 2000:  # 2 seconds
                    recommendations.append(f"Operation {operation} is slow (avg: {avg_time:.1f}ms). Consider optimization.")
        
        if not recommendations:
            recommendations.append("System performance looks good! No specific recommendations.")
    ...
    def compare_to_baseline(self, metric_name: str) -> Optional[Dict[str, Any]]:
        ...
        recent_values = [
            ...
            if m.name == metric_name and m.timestamp >= datetime.now() - timedelta(minutes=10)
        ]
