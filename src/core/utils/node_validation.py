...
def ensure_pydantic_model(*model_fields):
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            if not state:
                logger.error(
                    f"Function '{func.__name__}' called without state or kwargs"
                )
                raise ValueError(
                    f"Function '{func.__name__}' requires state parameter or kwargs"
                )
            ...
            for field_name, model_class in model_fields:
                if field_name in validated_state:
                    ...
                    if isinstance(field_value, dict):
                        try:
                            validated_state[field_name] = model_class.model_validate(
                                field_value
                            )
                            logger.debug(
                                f"Converted {field_name} to {model_class.__name__} model"
                            )
                        except ValidationError as e:
                            logger.error(
                                f"Failed to validate {field_name} as {model_class.__name__}: {e}"
                            )
                            raise
                    else:
                        logger.warning(
                            f"Field {field_name} is not a dict, cannot convert to {model_class.__name__}"
                        )
