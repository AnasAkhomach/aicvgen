...
class DependencyContainer(IDependencyProvider):
    ...
    def __init__(self, session_id: Optional[str] = None):
        ...
        self._creating: set = set()  # Track dependencies being created to prevent cycles
    ...
    def register(self, metadata: DependencyMetadata) -> None:
        ...
        with self._lock:
            if metadata.name in self._registrations:
                logger.warning(f"Dependency '{metadata.name}' already registered, overwriting")
            
            self._registrations[metadata.name] = metadata
            logger.debug(f"Dependency registered: {metadata.name}", 
                        scope=metadata.scope.value, lazy=metadata.lazy)
    
    def register_singleton(self, name: str, dependency_type: Type[T], 
    ...
    def register_transient(self, name: str, dependency_type: Type[T], 
                          factory: Callable[[], T], 
                          dependencies: Optional[List[str]] = None) -> None:
    ...
    def register_session(self, name: str, dependency_type: Type[T], 
                        factory: Optional[Callable[[], T]] = None, 
                        dependencies: Optional[List[str]] = None) -> None:
    ...
    def get_or_create(self, dependency_type: Type[T], name: Optional[str] = None, 
                     factory: Optional[Callable[[], T]] = None, 
                     scope: LifecycleScope = LifecycleScope.SINGLETON) -> T:
    ...
    def _get_existing_instance(self, name: str, metadata: DependencyMetadata) -> Optional[DependencyInstance]:
        ...
        try:
            ...
            for dep_name in metadata.dependencies:
                if dep_name in self._registrations:
                    ...
                    resolved_deps[dep_name] = self.get(dep_metadata.dependency_type, dep_name)
            ...
            logger.info(f"Dependency created successfully: {name}", 
                       creation_time=creation_time, scope=metadata.scope.value)
            ...
            self._error_handler.handle_error(
                error_msg, ErrorCategory.CONFIGURATION, ErrorSeverity.HIGH,
                context=ErrorContext(
                    additional_data={"dependency_name": name, "dependency_type": str(metadata.dependency_type)}
                )
            )
            raise
        finally:
    ...
    def _dispose_instance(self, dep_instance: DependencyInstance) -> None:
        ...
        try:
            ...
            logger.error(f"Error disposing dependency {dep_instance.metadata.name}: {e}")
    ...
    def dispose_session(self, session_id: str) -> None:
        ...
        with self._lock:
            if session_id in self._session_instances:
                ...
                logger.info(f"Session dependencies disposed", session_id=session_id, 
        ...
        with self._lock:
            ...
            for name, instance in self._instances.items():
                if (instance.metadata.max_idle_time and 
                    current_time - instance.last_accessed > instance.metadata.max_idle_time):
        ...
        with self._lock:
            return {
                **self._stats,
                ...
                "session_instances": sum(len(instances) for instances in self._session_instances.values()),
                "total_instances": len(self._instances) + sum(len(instances) for instances in self._session_instances.values())
            }
