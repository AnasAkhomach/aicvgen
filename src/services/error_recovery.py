...
class ErrorRecoveryService:
    ...
    def classify_error(self, exception: Exception, context: Dict[str, Any] = None) -> ErrorType:
        ...
        # API errors
        if any(keyword in error_message for keyword in [
            "api error", "invalid api key", "unauthorized", "401", "403"
        ]):
        ...
        # Content errors
        if any(keyword in error_message for keyword in [
            "content", "empty response", "no content", "invalid content"
        ]):
            return ErrorType.CONTENT_ERROR
        
        # System errors
        if any(keyword in error_message for keyword in [
            "system", "internal server error", "500", "503"
        ]):
    ...
    async def handle_error(
        ...
        base_action = self.error_strategies.get(error_type, self.error_strategies[ErrorType.UNKNOWN_ERROR])
    ...
    def _update_circuit_breaker(self, error_context: ErrorContext):
        ...
        if error_context.error_type not in [ErrorType.SYSTEM_ERROR, ErrorType.API_ERROR]:
            return
    ...
    def _calculate_delay(self, action: RecoveryAction, retry_count: int, error_context: ErrorContext) -> float:
    ...
    def _generate_fallback_content(self, item_type: ContentType, error_context: ErrorContext) -> str:
    ...
    async def execute_recovery_action(
        ...
        if action.strategy == RecoveryStrategy.FALLBACK_CONTENT:
            ...
            self.logger.warning(
                "Circuit breaker triggered, stopping processing",
                session_id=session_id
            )
            return False
    ...
    def cleanup_session(self, session_id: str):
        ...
        keys_to_remove = [key for key in self.circuit_breakers.keys() if key.startswith(session_id)]
