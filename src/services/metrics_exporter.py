...
LLM_TOKEN_USAGE_TOTAL = Counter(
    ...
    [
        "model_name",
        "token_type",
    ],  # Labels to differentiate by model and token type (input/output)
)
...
ACTIVE_SESSIONS_GAUGE = Gauge(
    "aicvgen_active_sessions", "Number of currently active user sessions."
)
...
def record_llm_request(
    ...
    if success:
        LLM_TOKEN_USAGE_TOTAL.labels(model_name=model_name, token_type="input").inc(
            input_tokens
        )
        LLM_TOKEN_USAGE_TOTAL.labels(model_name=model_name, token_type="output").inc(
            output_tokens
        )


def record_agent_execution(
    agent_name: str, duration: float, success: bool, error_type: str = None
):
