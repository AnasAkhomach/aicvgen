...
class ExecutiveSummaryWriterAgentInput(BaseModel):
    ...
    professional_experience: str = Field(
        description="Extracted professional experience from CV"
    )
    projects: str = Field(description="Extracted projects from CV")
    research_findings: Optional[dict] = Field(
...
def extract_agent_inputs(agent_name: str, state: "GlobalState") -> Dict[str, Any]:
    ...
    if "job_description" in model_fields:
        ...
        if agent_name == "ExecutiveSummaryWriter":
            ...
            agent_inputs["job_description"] = (
                job_desc_data.raw_text if job_desc_data else ""
            )
        else:
    ...
    if "generated_key_qualifications" in model_fields:
        agent_inputs["generated_key_qualifications"] = state.get(
            "generated_key_qualifications", []
        )

    if "generated_professional_experience" in model_fields:
        agent_inputs["generated_professional_experience"] = state.get(
    ...
    if "generated_executive_summary" in model_fields:
        agent_inputs["generated_executive_summary"] = state.get(
    ...
    if agent_name == "ExecutiveSummaryWriter":
        ...
        if structured_cv and "key_qualifications" in model_fields:
            ...
            agent_inputs["key_qualifications"] = (
                "\n".join([item.content for item in key_qual_section.items])
                if key_qual_section
                else ""
            )
        if structured_cv and "professional_experience" in model_fields:
            ...
            prof_exp_section = next(
                (
                    s
                    for s in structured_cv.sections
                    if s.name == "Professional Experience"
                ),
                None,
            )
            agent_inputs["professional_experience"] = (
                "\n".join([item.content for item in prof_exp_section.items])
                if prof_exp_section
                else ""
            )
        if structured_cv and "projects" in model_fields:
            ...
            projects_section = next(
                (s for s in structured_cv.sections if s.name == "Project Experience"),
                None,
            )
            agent_inputs["projects"] = (
                "\n".join([item.content for item in projects_section.items])
                if projects_section
                else ""
            )
    ...
    if agent_name == "KeyQualificationsWriter":
        if "main_job_description_raw" in model_fields:
            job_desc_data = state.get("job_description_data")
            agent_inputs["main_job_description_raw"] = (
                job_desc_data.raw_text if job_desc_data else ""
            )
